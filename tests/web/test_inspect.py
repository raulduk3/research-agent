...
INSPECTOR_SCOPES = frozenset({"runs:read", "submissions:read", "manifests:read"})
...
def test_the_agent_page_lists_only_its_configurations_runs_with_a_not_yet_stored_line(
    seed: tuple[Seed, Database, ArtifactStore], inspector_client: TestClient
) -> None:
    fixture, _, _ = seed
    sheet_hash = fixture.seal_sheet()
    ...
    assert response.status_code == 200
    assert "population record and resolutions: not yet stored" in response.text
    assert run_id in response.text
