...
| `StorageHandlers.get_runs_id_worker` | GET /v1/runs/{id}/worker (200) | no body, no query; `orchestrator` role, scope `runs:worker` | `{run_id:Id,configuration_id:Id,attempt:NonNegative,genome_hash:Sha256,snapshot_hash:Sha256,budgets:RunBudgets,allowed_tools:List<ToolName>,paper_id:Text,issued_question_ids:List<Id>,prompt:Text}`; the run's fields exactly as stored, `prompt` the `genome_parts` row of the run's `configuration_id`; 422 `unavailable_input` when there is none (#306) |
