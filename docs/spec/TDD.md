...
| `ToolService` | `storage: StorageClientHandle`, `models: ModelClientHandle`, `clock: ClockHandle` | `query_cards`, `neighbors`, `graph`, `deep_read`, `ask`, `submit`; each accepts its named ToolRequest and returns its matching ToolResult |
...
<!-- id: TDD-1.1.1 | implements: EN-12 | code: src/research_agent/outcomes/targets.py#registry | tests: tests/outcomes/test_targets.py | status: implemented -->
...
<!-- id: TDD-1.1.2 | implements: EN-13 | code: src/research_agent/outcomes/windows.py#OutcomeWindow | tests: tests/outcomes/test_windows.py | status: implemented -->
...
<!-- id: TDD-1.1.3 | implements: EN-15 | code: src/research_agent/outcomes/resolve.py#Resolver.resolve_target | tests: tests/outcomes/test_resolution.py | status: implemented -->
...
<!-- id: TDD-1.1.5 | implements: IN-12 | code: src/research_agent/outcomes/corrections.py#CorrectionService | tests: tests/outcomes/test_corrections.py | status: implemented -->
...
<!-- id: TDD-1.1.6 | implements: PL-11 | code: src/research_agent/ingest/pilot_run.py#main, src/research_agent/learning/pipeline.py#FitWorker | tests: tests/ingest/test_pilot_run.py, tests/learning/test_pipeline.py | status: pending:#65 -->

Checkpoint acquisition, extraction, identity reconciliation, automatic resolution, encoding and release assembly independently. Keys combine stage version, ordered input hashes and configuration hash. Verify temporary artifacts before committing the manifest. Interrupted label work reuses original papers, raw citation responses and embeddings. A failed target cannot expose a partial release or trigger unchanged paid acquisition. Head fitting (`bin/fit-heads`) is one `fit` job over an initial-fit release and the acquisition-pilot release, both named by content hash: it publishes and checkpoints one `TrainingArrays` per partition, reads each back through `learning/arrays.py#materialize_training_arrays`, then fits, calibrates by primary category, qualifies, and commits the qualification report, the bundle and the promotion decision. A resumed job reads its committed partitions back instead of rebuilding them. Only a target the bundle carries as qualified is scored for promotion, and the job activates nothing (#278). Test that a job killed after its partitions are committed resumes without rebuilding them, and that two runs in separate stores commit the same bundle bytes.
...
Accept X float32 [N,2d+m], Y boolean [N,3], M boolean [N,3], row ids and ordered manifests, X the embedding block followed by the declared metadata block of Appendix B: Learning protocol. Fit each logistic model on its own known rows with the exact objective and numeric settings in Appendix B: Learning protocol. Use float64 optimization, unpenalized intercept and plain numeric artifacts. Reject nonfinite inputs, mismatched target order, single-class support and incompatible representation ids. Numerical-gradient and mask-invariance tests exercise the real optimizer; no encoder weights change.

#### TDD-1.1.9 Representation-only feature assembly

<!-- id: TDD-1.1.9 | implements: FT-09 | code: src/research_agent/learning/features.py#assemble_features | tests: tests/learning/test_features.py | status: implemented -->
...
<!-- id: TDD-1.1.10 | implements: FT-10 | code: src/research_agent/learning/refresh.py#build_refresh | tests: tests/learning/test_refresh.py | status: implemented -->
...
<!-- id: TDD-1.1.11 | implements: FT-11 | code: src/research_agent/learning/calibration.py#fit_calibrator | tests: tests/learning/test_calibration.py | status: implemented -->
...
<!-- id: TDD-1.1.12 | implements: FT-17 | code: src/research_agent/contracts/learning.py#CombinedFeatureRecord, src/research_agent/learning/features.py#assemble_features | tests: tests/learning/test_features.py | status: implemented -->
...
Freeze the 100-paper pilot separately from the 2000-candidate modeling selection using the protocol month/week hash rules over canonical unversioned arXiv ids; cross-listed families of the configured categories are eligible and carry their primary category. Preserve all selected ids and shortfalls. If allowed, expand to 5000 before inspecting locked evaluation, retaining partition memberships. Join original features and automatic labels without conditioning inclusion on success. Group families and publication weeks, freeze temporal partitions and record reconstructed acquisition. Verify hashes and gates before publishing. Each row records the card fields of the declared metadata block (author, category, token and version counts, first-availability weekday, code link) counted with the pinned embedding tokenizer, and cites its automatic labels and its `CombinedFeatureRecord` by content hash, all published with the release (`learning/release.py#ReleaseWorker`). A release written before the card fields were recorded omits them and still parses; head fitting refuses its rows by name (`learning/features.py#row_card_metadata`, #278).

#### TDD-1.1.14 Shared automatic observation protocol

<!-- id: TDD-1.1.14 | implements: FT-19 | code: src/research_agent/outcomes/protocol.py#ObservationProtocol | tests: tests/outcomes/test_protocol.py | status: implemented -->
...
<!-- id: TDD-1.1.15 | implements: FT-20 | code: src/research_agent/outcomes/targets.py#validate_extension | tests: tests/outcomes/test_targets.py | status: implemented -->
...
<!-- id: TDD-1.1.16 | implements: FT-21 | code: src/research_agent/outcomes/resolve.py#Resolver.resolve_target | tests: tests/outcomes/test_resolution.py | status: implemented -->
...
<!-- id: TDD-1.1.17 | implements: FT-22 | code: src/research_agent/learning/qualification.py#qualify_corpus | tests: tests/learning/test_qualification.py | status: implemented -->
...
<!-- id: TDD-1.1.18 | implements: FT-23 | code: src/research_agent/learning/bundles.py#validate_bundle | tests: tests/learning/test_bundles.py | status: implemented -->
...
<!-- id: TDD-1.1.20 | implements: FT-25 | code: src/research_agent/artifacts/lineage.py#apply_correction | tests: tests/artifacts/test_lineage_corrections.py | status: implemented -->
...
The shared tool service allocates a monotonic per-run call sequence through storage before executing each request. Trace entries contain run_id, call_id, request_hash, schema decision, start/end timestamps, response_hash, retrieved artifact ids and budget deltas; refused calls are entries too. Append a terminal response or error event rather than editing the request event. Each entry also stores the bytes its hash covers: the canonical request `{run_id, snapshot_id, tool, arguments}` exactly as the run made it, and the canonical envelope the run received. The service sends them with the request and terminal commands, and storage writes each as an artifact of kind `trace_request` or `trace_response` in the same transaction as its row, at most 256 KiB each. A larger payload arrives cut to 256 KiB and is stored flagged as truncated, under the hash of the stored bytes, while the row keeps the hash of the whole. `GET /v1/runs/{id}/trace` (role `owner`, scope `owner:read`) returns the run's entries in call order with both payloads resolved; any other role is refused 403. Sealing requires successful complete receipts for cited retrievals and no unresolved earlier tool call. The current submit request is exempt from that prior-receipt test: its terminal receipt commits atomically with forecasts, nominations and terminal run state. A pending earlier call still blocks sealing. Agent-written summaries have no authority over trace fields. Exercise the actual HTTP tool path with an invented read in agent text and confirm conduct inspection reports only the separately captured retrieval events.
...
<!-- id: TDD-2.1.5 | implements: SR-05 | code: src/research_agent/tools/admission.py#admit_request | tests: tests/tools/test_request_admission.py | status: pending:#73 -->
...
Resolve horizon metadata from each issued question's immutable definition: verified first-public origin, its target definition's own fixed event end (365, 180 or 60 days) and its separate maturity allowance. The agent does not supply or choose a horizon; strict submit parsing rejects a horizon override as an extra field. Accepted ledger forecasts persist the derived interval and definition hash alongside their question id. An unresolved or inconsistent question manifest prevents acceptance of the entire attempt rather than inventing timing. Tests reject a 455-day question event window and an agent horizon override, verify a valid accepted forecast stores the exact question-derived 365-day interval, and verify the same for a 60-day and a 180-day question, plus an event window outside the admitted set. `ADMITTED_EVENT_WINDOW_DAYS = (365, 180, 60)` covers the three citation targets and the two short-horizon definitions (`early_citation_rank_60d`, `venue_180d`); `validate_horizon` takes its caller-resolved `event_window_days` rather than assuming the citation targets' 365 days for every question.
...
<!-- id: TDD-2.1.12 | implements: SR-11 | code: src/research_agent/storage/submissions.py#accept_submission | tests: tests/storage/test_submissions.py | status: implemented -->
...
<!-- id: TDD-2.1.14 | implements: SR-12 | code: src/research_agent/platform/isolation.py#WorkerIsolation | tests: tests/platform/test_worker_isolation.py | status: implemented -->
...
<!-- id: TDD-2.1.15 | implements: SR-13 | code: src/research_agent/platform/network.py#EgressManifest | tests: tests/platform/test_external_egress.py | status: implemented -->
...
<!-- id: TDD-2.1.17 | implements: SR-15 | code: src/research_agent/orchestration/stamps.py#build_run_stamp | tests: tests/orchestration/test_run_stamp.py, tests/orchestration/test_stamps.py | status: implemented -->

Before issuing worker credentials, resolve the snapshot's paper-card manifest through storage and read every producing prediction-head bundle id from the pinned papers' own cards, never from the active model pointer; combine them with the caller's fixed genome hash, seed, agent-model manifest and service-image versions into one immutable stamp. An unresolved snapshot or an unpinned requested paper blocks the run from starting. The run record carries the stamp as its `model_identity`, one bundle id or null per target (`RunStamp.model_identity`); a target whose pinned cards name two bundles refuses the run as `mixed_prediction_head_bundles:<target>` rather than recording either. The caller's fixed half comes from `orchestration/bindings.py#current_bindings`: the pinned agent-model manifest, each observed service role's image digest, and the launch profile's `run` budgets and tools; no observed image, or two for one role, refuses with a named reason (#285). `bin/bindings` prints that half for `bin/daily --bindings` (#317): the agent-model manifest is the canonical hash of the deployment manifest `bin/run-agent` pins (`orchestration/bindings.py#agent_model_manifest_hash`: the profile's provider and model, the endpoint and the revision the provider reported, and the profile's qualification flag), the image digests are the operator's observed role-to-digest map, and the index identities are the latest sealed snapshot's, in its order (`current_index_identities`), refused as `no_sealed_snapshot` before the first seal. The first day binds `--namespace DIR` instead: a published representation namespace's identity is the canonical SHA-256 of the manifest `bin/import-embeddings` records in it, the representation manifest less its qualification flag with the chunk policy, never of its vectors (`retrieval/passages.py#namespace_identity`); import prints it last, `--print-identity` prints it alone, and a namespace with no manifest or an incomplete one is refused (#355). Test queued snapshot A after active bundle B promotion and verify every producing-model reference remains A.

#### TDD-2.1.18 Externally immutable chain-head receipts

<!-- id: TDD-2.1.18 | implements: SR-16 | code: src/research_agent/storage/anchors.py#AnchorClient, src/research_agent/storage/anchors.py#HttpsAnchorTransport, src/research_agent/storage/anchors.py#AnchorBindingRepository, src/research_agent/platform/anchoring.py#AnchorScheduler | tests: tests/storage/test_anchor_receipts.py, tests/storage/test_anchors.py | status: implemented -->

Storage tracks last_receipted_sequence and last_receipted_at and schedules anchoring after 100 new records or 15 minutes. Send sequence, record hash, profile id and an idempotency key to the separate receiver; retain its authenticated receipt as an immutable artifact. Receiver authority permits append/read only and rejects replacement or decreasing sequence. A timeout does not advance the acknowledged watermark. Sealing checks whether unanchored prospective records exceed the 30-minute backlog bound and refuses new seals while preserving capture. Test receiver outage/recovery and a recomputed altered ledger copy against an independently retained receipt. The receiver transport (`HttpsAnchorTransport`) POSTs the head to the receiver's `/v1/anchors` over TLS pinned to the receiver's CA, authenticated by exactly one of a client certificate or a bearer token file, and accepts only a 200 or 201 receipt for the exact sequence and hash sent; an unanswered request is a timeout. `anchor_bindings(receiver_url, verified_at, last_acknowledged_sequence, last_acknowledged_hash, last_acknowledged_at)` records the bound receiver; a trigger refuses a delete, a changed receiver, and an acknowledged head that moves back or changes hash at the same sequence. `python -m research_agent bind-anchor --receiver URL --verify --storage-config PATH` sends the committed head once and records the binding only on its acknowledgement. When the storage configuration has an `anchor` section, `serve-storage` refuses to start unbound and runs `AnchorScheduler`, which every 30 seconds submits the head when `is_anchoring_due` and advances the acknowledged head only on a receipt.
...
<!-- id: TDD-2.1.20 | implements: SR-17 | code: src/research_agent/platform/readiness.py#LayerAdmission, src/research_agent/platform/reports.py#admit_activation | tests: tests/platform/test_layer_admission.py, tests/platform/test_activation_readiness.py, tests/storage/test_qualification_reports.py | status: implemented -->

Maintain an immutable admission record naming layer id, baseline configuration hash, candidate hash, registered primary metric, comparison report ids and activation scope. The gate verifies baseline/candidate comparability and temporal precedence from stored registration and execution provenance; it does not treat a green unit test as a measured baseline. The Jev layer is admitted to the launch by decision 0024 (#238) and denied unless the launch profile's `jev_admitted` is set, which follows from the RD-22 smoke test, RD-23 preregistration and RD-24 readiness record (#62), not from the decision. Future prediction heads remain denied irrespective of a caller flag. The comparison reports come from storage: each qualification report (inference battery, Jev smoke, retrieval, three-head) is stored as an artifact by the command that produces it, with a `qualification_report_recorded` ledger event naming its kind, primary metric and registration and execution instants, and admission reads the latest report of each kind and denies by name every kind storage lacks. Tests reject a missing baseline, wrong-metric report, post-hoc registration and a Jev admission request while `jev_admitted` is unset. Tests admit over a full stored set and deny a set missing its retrieval report, naming it.
...
<!-- id: TDD-2.1.22 | implements: SR-19 | code: src/research_agent/storage/versions.py#SupersessionRecord | tests: tests/storage/test_supersession.py | status: implemented -->
...
<!-- id: TDD-2.1.23 | implements: SR-20 | code: src/research_agent/contracts/verification.py#VerificationRecord | tests: tests/contracts/test_verification_registry.py | status: implemented -->
...
<!-- id: TDD-2.1.25 | implements: SR-28 | code: src/research_agent/platform/profile.py#LaunchProfile | tests: tests/platform/test_profile_readiness.py, tests/platform/test_profile.py | status: implemented -->

Parse one closed, immutable launch profile into runtime, storage, model, source, budget, evaluation, privacy, recovery and disabled-capability groups, plus the `run` section of Appendix A's per-run budgets and allowed tools that every run record carries, whose launch values a test holds equal to the loop's enforced ceilings (#285). Separate deployment bindings and operator funding authorizations from chosen design ceilings. Readiness returns a typed list of unmet gates for collection, engineering or study; collection needs licensed source/storage bindings, engineering adds local model/replay integrity, and study additionally needs all qualification, backup and funded inference prerequisites. Every job/artifact records profile_hash and mode. Unknown override keys are rejected. Parameterized deletion tests remove each required group; integration tests prove collection can operate while study is refused for an unfunded endpoint.
...
<!-- id: TDD-2.1.34 | implements: PL-06 | code: src/research_agent/platform/builds.py#BuildManifest, src/research_agent/platform/builds.py#ImageRecord, src/research_agent/platform/producer.py#source_commit | tests: tests/platform/test_build_manifest.py, tests/platform/test_builds.py, tests/platform/test_producer.py | status: implemented -->

The build manifest includes source tree hash, Python/tool versions, uv lock hash, base-image digest, package hashes and selected model/runtime identities. Fail unresolved tags, unconstrained dependencies or missing lock artifacts before producing a releasable image. Embed product version and manifest hash as OCI labels; record the running image digest separately because a human tag is mutable. Run stamps inspect actual selected containers rather than trusting build configuration. Tests reject a floating base image and altered lock hash, then start containers and verify their observed image digests and labels appear in the run stamp. `bin/build-image` refuses a dirty tree, builds the `Dockerfile` with those labels and records the engine-reported digest, source commit and manifest in `deploy/images.json` (`ImageRecord`), which recomputes the manifest hash on read; `deploy/compose.yaml` selects every application service by that digest (#316). The build passes that source commit as the build argument `RESEARCH_AGENT_SOURCE_COMMIT`, which the `Dockerfile` sets in the image environment; an operator command records `platform/producer.py#source_commit` as its producer commit, which is that variable when set, else the checkout's `HEAD`, else a refusal that names both, so a command in the image, which carries neither `git` nor the repository, names the recorded commit (#366).

#### TDD-2.1.35 Runtime-only scoped secret files

<!-- id: TDD-2.1.35 | implements: PL-07 | code: src/research_agent/platform/secrets.py#SecretBindings | tests: tests/platform/test_secret_injection.py | status: implemented -->
...
One shared tools service resolves each authenticated run specification and snapshot from storage. It keeps no mutable conversation state; durable call budgets, retrieved ids and trace events belong to storage with atomic reservation before work. Cache only immutable content keyed by snapshot_hash, representation_hash, tool version and normalized arguments, and reapply authorization before returning cached bytes. Submission uses the same service but is forwarded to the storage-owned commit boundary through the sealer. Concurrent integration tests seed distinguishable run-specific arguments and verify no response, error or budget count leaks into the other run. The service's own listener is `tools/http.py#create_tool_server`: over mutually authenticated TLS, `POST /v1/calls` takes one forwarded call (`run_id`, `snapshot_id`, `tool_call_id`, `tool` and the call envelope as its JSON text, no other field) and answers it through `ToolService` with the response envelope and the budget deltas the loop charges; an unadmitted certificate is 401, a malformed request 422 with nothing traced, and `GET /health` reports readiness to an admitted caller. The run worker's `tools/client.py#ToolServiceClient` sends each call once and never retries it, since the service records a call before it runs (`tests/tools/test_tool_http.py`, #323). No command starts the listener yet.

#### TDD-2.1.37 Snapshot-keyed indexes without latest fallback

<!-- id: TDD-2.1.37 | implements: PL-21 | code: src/research_agent/tools/snapshots.py#SnapshotIndex | tests: tests/tools/test_snapshot_pinning.py | status: implemented -->
...
The shared model service owns the single serving instance of the frozen embedding model and qualified numeric prediction-head bundles. Reader and tools call typed embedding/prediction endpoints; workers have no route or token. Requests name representation/bundle identity and return producing identity with output. The tool service embeds a search query through `POST /v1/embeddings/query` (`models/service.py#create_model_server`, `models/client.py#ModelServiceClient`, tested in `tests/models/test_service_client.py`) over mutually authenticated TLS, under the `search_query: ` prefix; a request naming another representation is 409 `representation_mismatch` and an unadmitted certificate 401 (#287). On the same listener `GET /health` answers an admitted caller with the held embedder's identity and bundle hash, and 503 once the service is closed; `serve-models` starts it from a launch configuration whose profile pins the embedder revision (`platform/services/models.py#build_model_server`, tested in `tests/models/test_service_health.py` and `tests/platform/test_launchers.py`, #315). Batch jobs may fit numeric prediction heads but request encoding from the same service rather than loading another serving embedding model. A missing model process or incompatible manifest yields unavailable, not local fallback. Compose inspection plus request tracing in a multi-worker acceptance test verifies one serving process and that all returned model artifacts originated from it.

#### TDD-2.1.39 Weight-free worker images and mounts

<!-- id: TDD-2.1.39 | implements: PL-09 | code: src/research_agent/platform/workers.py#WorkerImagePolicy | tests: tests/platform/test_weight_free_workers.py | status: implemented -->
...
<!-- id: TDD-2.1.40 | implements: PL-10 | code: src/research_agent/platform/preflight.py#HostFloorReport | tests: tests/platform/test_host_floor.py | status: implemented -->
...
<!-- id: TDD-2.1.45 | implements: PL-18 | code: src/research_agent/storage/persistence.py#PersistentStores | tests: tests/storage/test_container_replacement.py | status: implemented -->
...
<!-- id: TDD-2.1.46 | implements: PL-19 | code: src/research_agent/platform/network.py#ReachabilityPolicy | tests: tests/platform/test_reachability_matrix.py | status: implemented -->
...
<!-- id: TDD-2.1.47 | implements: PL-22 | code: src/research_agent/web/auth.py#RaterSession | tests: tests/web/test_private_rater_access.py | status: implemented -->
...
<!-- id: TDD-3.1.1 | implements: EN-01 | code: src/research_agent/ingest/daily.py#eligible_families | tests: tests/ingest/test_daily.py | status: pending:#65 -->

Parse arXiv records into family_id, version_id, categories, first_public_at, captured_at and source_hash; strip version suffix only through the canonical identity adapter. A fetch manifest lists every page and completion token. Stage records through storage, then commit corpus membership only after complete pagination and validation. Include a family when its category set intersects the configured corpus categories, cs.AI, cs.LG, quant-ph and q-bio by default, and record the primary category of its earliest public version; cross-listing does not duplicate it. Keep older referenced works in graph-reference identity records, not corpus membership. Verify a real storage transaction with an interrupted second page leaves membership unchanged, an irrelevant category is excluded, duplicate category hits create one family and a cross-listed family carries its primary category.

#### TDD-3.1.2 Prospective eligibility at seal and resolution

<!-- id: TDD-3.1.2 | implements: EN-02 | code: src/research_agent/forecasts/eligibility.py#check_prospective | tests: tests/forecasts/test_eligibility.py | status: implemented -->
...
<!-- id: TDD-3.1.3 | implements: EN-35 | code: src/research_agent/snapshots/documents.py#DocumentPins | tests: tests/snapshots/test_document_pins.py | status: pending:#73 -->
...
<!-- id: TDD-3.1.6 | implements: EN-38 | code: src/research_agent/ingest/discovery.py#capture_daily_picks | tests: tests/ingest/test_discovery.py | status: pending:#65 -->
...
<!-- id: TDD-3.1.8 | implements: EN-04 | code: src/research_agent/storage/resolutions.py#append_resolution | tests: tests/storage/test_resolutions.py | status: implemented -->
...
<!-- id: TDD-3.1.11 | implements: EN-07 | code: src/research_agent/ingest/capture.py#preserve_response | tests: tests/ingest/test_capture.py | status: pending:#65 -->
...
<!-- id: TDD-3.1.12 | implements: EN-08 | code: src/research_agent/storage/resolutions.py#validate_resolver_identity | tests: tests/storage/test_resolver_identity.py | status: implemented -->
...
<!-- id: TDD-3.1.13 | implements: EN-09 | code: src/research_agent/ingest/daily.py#run_once, src/research_agent/orchestration/daily.py#issue_day | tests: tests/integration/corpus/test_daily_ingest.py, tests/orchestration/test_daily.py | status: pending:#73 -->

After a completed daily ingest, use its immutable membership manifest to select first-public eligible families without sorting on predicted success. Route each family to the island of its primary category, then within each island sort by first_public_at then family_id; a paper carries its island and never mixes two. Record excluded late arrivals explicitly. Build question ids from family and qualified target-definition hashes; each paper and its one slot per active configuration of its island reference one parent snapshot, sealed after the day's sheets and pinned to each (decision 0027). `orchestration/daily.py#issue_day` drives `run_once` and writes one run record per sampled paper and active configuration, every command keyed by its content so a repeated day creates nothing. Storage enforces unique UTC processing day and idempotent build identity. Test 0, 1, 20, 21 and 1001 papers across three islands, duplicate scheduler calls, a cross-listed family routed by its primary category, and exact one-slot-per-configuration coverage of each island's papers at both the seeded island size and the floor of four.
...
<!-- id: TDD-3.1.16 | implements: EN-14 | code: src/research_agent/outcomes/results.py#ResolutionResult | tests: tests/outcomes/test_results.py | status: implemented -->
...
<!-- id: TDD-3.1.17 | implements: EN-16 | code: src/research_agent/evolution/policy.py#selection_disposition | tests: tests/evolution/test_selection_disposition.py | status: implemented -->
...
<!-- id: TDD-3.1.25 | implements: EN-24 | code: src/research_agent/forecasts/admission.py#validate_target | tests: tests/forecasts/test_admission.py | status: implemented -->
...
<!-- id: TDD-3.1.26 | implements: EN-25 | code: src/research_agent/forecasts/admission.py#validate_target | tests: tests/forecasts/test_admission.py | status: implemented -->
...
<!-- id: TDD-3.1.27 | implements: EN-26 | code: src/research_agent/forecasts/admission.py#validate_target | tests: tests/forecasts/test_admission.py | status: implemented -->
...
<!-- id: TDD-3.1.28 | implements: EN-27 | code: src/research_agent/forecasts/admission.py#validate_target | tests: tests/forecasts/test_admission.py | status: implemented -->
...
<!-- id: TDD-3.1.29 | implements: EN-30 | code: src/research_agent/forecasts/admission.py#validate_issued_question | tests: tests/forecasts/test_issued_questions.py | status: implemented -->
...
<!-- id: TDD-3.1.30 | implements: EN-31 | code: src/research_agent/forecasts/admission.py#admit_registry | tests: tests/forecasts/test_registry_admission.py | status: implemented -->
...
<!-- id: TDD-3.1.33 | implements: EN-34 | code: src/research_agent/ratings/forecasts.py#seal_human_forecast | tests: tests/ratings/test_human_forecasts.py | status: implemented -->
...
Before queuing an island's slots for the day, draw its coverage sample by ascending SHA-256 of the canonical JSON object {utc_day, island, family_id, seed} with a recorded seed, taking the largest prefix of that order the island's share of the remaining authorized spend covers at the measured per-run cost times its active configurations; every genome of the island receives the identical sample, and coverage is recorded. The key names the day, not a sheet, so the draw needs nothing a sheet fixes (decision 0027); `orchestration/daily.py#issue_day` shares the day's spend among islands in proportion to their carded papers, fixes the seed from the profile hash and publishes each draw. Remaining authorized spend is `orchestration/bindings.py#remaining_spend`: zero unless funded execution is enabled, otherwise the smaller of the daily and monthly caps less the owner cost read's priced spend and each unpriced settled run's full reservation, never negative (#285). Read queued slots from storage in earliest paper seal-deadline then slot-id order; `bin/daily` records the day's run ids in that order (`IssuedDay.run_order`) and `bin/bindings --runs DAY` lists them, refusing a day whose issue did not finish as `day_not_issued:<day>` (#317). Acquire durable fenced leases and ask the operator-owned launcher for the predefined unprivileged worker specification, with at most two active workers. Never expose a Docker socket to workers. Each slot ends completed, void or missed_deadline; a restarted scheduler reconciles live workers against leases before launching anything. No retry slot is created after ambiguous model execution. Test two competing schedulers and a crash after launch acknowledgment using actual storage: no duplicate slot execution, third worker or silently dropped deadline; test that two genomes of one island receive the identical sample for a fixed seed and that recorded coverage matches the sample size.
...
<!-- id: TDD-3.1.48 | implements: AG-08 | code: src/research_agent/agents/loop.py#run_conversation, src/research_agent/agents/runner.py#run_agent | tests: tests/agents/test_loop.py, tests/agents/test_runner.py | status: implemented -->
...
<!-- id: TDD-3.1.49 | implements: AG-09 | code: src/research_agent/tools/dispatch.py#dispatch_tool, src/research_agent/tools/ask.py#AskHandler, src/research_agent/storage/assessments.py#AskRepository, src/research_agent/agents/runner.py#build_tool_service | tests: tests/tools/test_dispatch.py, tests/tools/test_ask_handler.py, tests/agents/test_runner.py | status: implemented -->

The dispatcher table contains query_cards, neighbors, graph, deep_read, ask and submit only. The worker presents the run's admitted subset; the shared tool service independently checks that subset from run_id, never trusts supplied names. Read handlers resolve only snapshot-bound artifacts. submit invokes the transactional storage command through the authorized service adapter. Unknown tool names return tool_not_allowed before execution and consume one call. Test shell/browser/HTTP/protected-write requests, tool aliases and a hidden seventh registration are all refused. `AskHandler` answers ask from Jev inside the tool service, which alone holds the Jev transport: it resolves the passage or card section from the run's snapshot and reaches storage only through the `tools` client under `jev_asks:write`: `POST /v1/runs/{id}/asks/reservations` reserves the ask on `storage/assessments.py#AskRepository` with the request bytes, `/settlements` settles it with the response bytes, and storage commits both as `tool_request` and `provider_response` artifacts in the reservation's and settlement's own transactions, as the trace does; `/answers` keeps the answer once per run and request in `jev_ask_answers`, and `GET /v1/runs/{id}/asks[/{work_key}]` reads the run's answered count and a kept answer, so a replayed call is served it without a provider call. A settlement touches only an ask reservation, and a reservation needs a run that allows ask. `agents/runner.py#build_tool_service` adds the handler when `tools/http.py#jev_from_environment` finds `JEV_PROVIDER_CONFIG` and `JEV_API_KEY` in the service's environment; the credential is never a flag. Test each kind against recorded Jev answers, the fifth ask and the spent pool refused with no provider call, a replay with no provider call, and an about outside the snapshot refused `not_in_snapshot`.
...
<!-- id: TDD-3.1.59 | implements: AG-28 | code: src/research_agent/agents/messages.py#prepare_request | tests: tests/agents/test_messages.py | status: implemented -->
...
<!-- id: TDD-3.1.62 | implements: AG-29 | code: src/research_agent/agents/transcript.py#record_exchange | tests: tests/agents/test_transcript.py | status: implemented -->
...
<!-- id: TDD-3.1.63 | implements: AG-30 | code: src/research_agent/agents/transcript.py#record_images | tests: tests/agents/test_transcript.py | status: implemented -->
...
<!-- id: TDD-3.1.71 | implements: AG-24 | code: src/research_agent/agents/messages.py#assemble_system_prompt | tests: tests/agents/test_messages.py | status: implemented -->

The prompt builder accepts only the immutable configuration and common prompt-schema manifest; scheduling checks exclusion state outside that API. A run's system message is its genome's prompt followed by its scan policy, read policy and probability assignment rule, in that order under the labels `Scan policy`, `Read policy` and `Probability assignment rule` (`agents/messages.py#assemble_genome_system_prompt`), and the rendered whole is held to the 16000-character bound and the exclusion-term rule; a change to one policy part alone changes the message bytes and the run's recorded request hash (#322). It never reads exclusion events, score reports or future outcome projections. Validate admitted prompt text contains no exclusion-action terminology required forbidden by the SDD, rejecting rather than editing it. Test byte-identical assembly for the same configuration before and after run/configuration quarantine, while scheduler authority independently blocks the latter.
...
<!-- id: TDD-4.1.8 | implements: IN-08 | code: src/research_agent/scoring/baselines.py#base_rate_baseline_answers | tests: tests/scoring/test_baselines.py | status: deviation:#75 -->

Read each qualified target bundle's immutable fitting positive and known counts for the paper's primary category; compute numerator/denominator and seal that value under a dedicated baseline submitter. Reject zero denominator, unqualified target, unknown category and changed target definition. Add a later label, a different target's label and a different category's label to storage and prove an existing batch forecast and its denominator remain byte-identical.
...
<!-- id: TDD-4.1.42 | implements: RD-02 | code: src/research_agent/contracts/cards.py#HeadCardValue | tests: tests/reader/test_cards.py | status: implemented -->
...
<!-- id: TDD-4.1.43 | implements: RD-03 | code: src/research_agent/reader/cards.py#assemble_card | tests: tests/reader/test_cards.py | status: implemented -->
...
Render a fixed ordered text schema: identity and abstract/source-span locator, coverage, a section map, three named prediction-head outputs, eight Jev fields, earlier neighbors and graph/count diagnostics. Each value carries its unit or scale, its availability reason and the one line that says where it comes from; hashes, instants other than first public availability, identity kinds, manifest and report ids and schema fields do not change how a value is read and stay on the stored `PaperCardBody`, off the text (#269). The section map says where to deep-read: each top-level section of the paper's passage records in document order, with its passage count and its range in the paper's passage numbering (from 1, ordered by section then passage), at most the first 40 then a count of the rest; a section holding only omitted blocks never appears, and a paper whose passages sit under fewer than two top-level sections renders the one line `(no section structure)` (#270). Enforce the 3000 embedding-token cap using the pinned tokenizer and explicit source-span fallback for an overlong abstract rather than silent truncation. Store rendered bytes once; tool output returns those bytes plus separate query evidence. Golden content tests compare actual text and ensure no binary/pointer-only paper card is accepted.

#### TDD-4.1.45 Vector-free tool projection

<!-- id: TDD-4.1.45 | implements: RD-05 | code: src/research_agent/reader/projections.py#AgentCardProjection | tests: tests/reader/test_projections.py | status: implemented -->
...
<!-- id: TDD-4.1.46 | implements: RD-14 | code: src/research_agent/reader/projections.py#strip_discovery_origin | tests: tests/reader/test_projections.py | status: implemented -->
...
<!-- id: TDD-4.1.47 | implements: RD-06 | code: src/research_agent/models/neighbors.py#earlier_neighbors | tests: tests/models/test_neighbors.py | status: implemented -->
...
<!-- id: TDD-4.1.48 | implements: RD-07 | code: src/research_agent/models/neighbors.py#neighbor_distance | tests: tests/models/test_neighbors.py | status: implemented -->
...
<!-- id: TDD-4.1.52 | implements: RD-13 | code: src/research_agent/models/neighbors.py#reference_centroid_distance | tests: tests/models/test_neighbors.py | status: implemented -->
...
Encode the exact SDD eight rows of each rubric version as immutable field ids in that version's order. A `choice` row carries ordered categories, full criteria and development-only positive/boundary examples; a `score` row its criteria as an ordered list, one per scale point; a `noul` row its instructions and the statement they ask about. Compute a canonical rubric hash and send one question per row in its primitive regardless of contribution type: a `choice` maps options to criteria, a `score` lists its criteria and a `noul` carries instructions only. `Rubric.launch()` is `jev-rubric-v2`; `Rubric.v1()` keeps `jev-rubric-v1`'s body and hash byte-for-byte, so its stored choice questions keep their untyped form. Reject unknown keys, extra questions, a field of another version, a criterion count that is not the field's scale, missing criteria and an altered body under a reused version. Fixture tests compare the full outbound schema of both versions with their approved rubric artifacts and deny run-role mutation.
...
Use a discriminated union: available carries the eight field results of one rubric version, each in its primitive; unavailable carries reason and no probabilities. A `choice` result carries category, ordered probability map and optional provider confidence; a `score` result its score, the legend and one probability per scale point with optional confidence; a `noul` result its probability and statement. Decode the provider's three wire shapes against the request's rubric: exactly its eight fields, each in its question's primitive, all expected categories or scale points, a `score` legend equal to the question's criteria, finite nonnegative probabilities summing to one within the provider's two-decimal rounding (0.05), a `noul` in [0,1], confidence in [0,1] when supplied and selected category or score membership. Retain low confidence and rubric categories such as insufficient-information as valid answers. A stored field map decodes under the rubric version its field ids name, so a v1 result still loads. Test NaN, missing categories or points, a mismatched legend, a noul outside [0,1], malformed sums and timeout without renormalization or fabricated answers.
...
Materialize the fixed 20-paper, one-per-week hash sample with its shortfall before any request. Send each paper's complete eight-field request under the operating limits, persist every request and response, and record per field the valid-result count, answer counts and unavailable reasons, with input coverage, latency and cost. Answer counts are `category_counts` ordered pairs: a `choice` field counts its categories, a `score` field its scale points `"0"` upward, and a `noul` field ten decile buckets of its probability, `[0.0,0.1)` to `[0.9,1.0]`; a v1 report's shape is unchanged. Pass requires at least 18 valid results per field and a recorded owner review; the report claims no accuracy. Tests refuse activation when one field has 17 valid results, when the owner review is missing and when the active provider identity differs from the report's, and check that every available paper-card assessment carries the unqualified label.
...
<!-- id: TDD-4.1.62 | implements: RD-24 | code: src/research_agent/assessments/readiness.py#check_assessment_readiness | tests: tests/assessments/test_readiness_assessments.py | status: implemented -->
...
<!-- id: TDD-4.1.64 | implements: MD-02 | code: src/research_agent/learning/policy.py#validate_training_origin | tests: tests/learning/test_policy.py | status: implemented -->
...
#### TDD-3.1.78 Agent paper requests and their acquisition

<!-- id: TDD-3.1.78 | implements: EN-44 | code: src/research_agent/tools/snapshot.py#authorize_snapshot, src/research_agent/storage/requests.py#PaperRequestRepository, src/research_agent/ingest/requests.py#acquire_requests, src/research_agent/snapshots/compose.py#seal_next_snapshot | tests: tests/tools/test_dispatch.py, tests/storage/test_requests.py, tests/ingest/test_requests.py, tests/snapshots/test_compose.py | status: implemented -->

Tool side (#264). `contracts/tools.py` adds a `not_in_snapshot` answer for `deep_read` and `graph` carrying the family id and a receipt of `requested`, `already_requested` or `request_budget_exhausted`. When snapshot membership (TDD-3.1.50) lacks the named family, the tool service records a `paper_requests` row {family_id, run_id, snapshot_hash, requested_at, status requested/acquiring/acquired/failed/refused, reason} through a worker-role storage route, unique on family while the status is requested or acquiring, with one ledger event per row, and answers with the receipt; a family with an open row answers `already_requested` and records nothing. `MAX_PAPER_REQUESTS_PER_RUN = 3` counts the run's `requested` receipts; the next call answers `request_budget_exhausted` and records a refused row. The answer consumes one call like any tool error and the receipt is in the run trace, so replay reproduces it. A run has no route that reads or alters the table.

Acquisition side (#265). An ingest-role stage lists open rows oldest first, ties by family id, and starts at most `MAX_ACQUISITIONS_PER_UTC_DAY = 200`; each row past it becomes refused with reason `request_budget_exhausted`. A started row becomes acquiring and enqueues the existing documents job under the arXiv request rules of Appendix A: Launch profile; after it commits, the reader extracts and stores the extraction record as an artifact whose content hash is the card's `extraction_hash`, so a snapshot's extraction read resolves it for `deep_read` (#304), passages are built and embedded on the host model service, `retrieval/passages.py#publish_index` publishes and `reader/cards.py#assemble_card` builds the card, and the family's citation edges are recorded from the snapshot channel where they exist. The card is built with the paper's own overview vector and the overview vectors of the snapshot's pinned items, read from their published index entries, so its earlier neighbors, neighbor distance and reference-centroid distance are computed (TDD-4.1.47, TDD-4.1.48, TDD-4.1.52); its parsed reference count is the extraction's bibliography entries, never fewer than the distinct cited families. The paper record's provenance carries the request id and marks the paper requested, so population selection, prediction-head training arrays and qualification sets exclude it. The row becomes acquired, or failed with the reason and no card. The next snapshot seal includes acquired papers; earlier snapshot manifests are never rewritten. Tests: a first and a repeated call for one absent family (requested, then already_requested with nothing recorded); a fourth distinct absent family in one run refused past the cap; a run-role read of the table refused; an open row carried through to a card with provenance naming it and present in the next snapshot while the earlier snapshot hash is unchanged, its extraction read back from that snapshot under the card's `extraction_hash`; a fetch failure leaving a failed row and no card; the 201st row of a day refused and recorded; a requested paper with two earlier pinned neighbors and three cited families, one without a vector, carded with both neighbors, both distances and a reference count of three.

#### TDD-4.1.79 Preference credit

<!-- id: TDD-4.1.79 | implements: IN-43 | code: src/research_agent/measurement/preference.py#credit_ratings, src/research_agent/storage/preference.py#PreferenceRepository | tests: tests/measurement/test_preference.py, tests/contracts/test_preference_credit.py, tests/storage/test_preference_credit_storage.py | status: implemented -->

For each rating event of the week, read the digest entry's `origin` and, for a population entry, every accepted submission of that island's genomes that nominated the paper with the citation_reach_365d probability each sealed for it, taken from the sealed forecast the nomination is linked to. Credit sign is +1 for like, -1 for dislike, 0 for skip; each nominating genome of the rated island receives sign times its probability divided by the sum of the nominating genomes' probabilities. A skip, a control entry and a service entry credit nothing and record no row. Persist `PreferenceCredit` {rating_id, genome_hash, island, entry_id, sealed_probability, share, iso_week} through storage, one immutable row per rating and genome (migration 0014), refusing a rating of a control or service entry, a genome of another island and a second credit for one rating. When any nominating submission of a population entry cannot be read, or no nominator belongs to the island, or the probabilities sum to zero, record no credit for that rating and persist the gap with its reason; never credit the readable subset. The scorer's projection (TDD-4.1.2) and resolver inputs have no path to this record. Test two nominators with unequal probabilities, a skip, a control entry, an unreadable nomination and byte-identical scoring inputs with and without ratings.

#### TDD-4.1.80 Weekly island report columns

<!-- id: TDD-4.1.80 | implements: FT-26 | code: src/research_agent/measurement/weekly.py#island_report, src/research_agent/web/report/app.py#create_app | tests: tests/measurement/test_island_report.py, tests/web/test_report.py | status: implemented -->

Build one report artifact per island at the weekly freeze: one row per genome present at the freeze, read from the population store of TDD-3.1.60, with per-target skill from TDD-4.1.75 (unavailable where none exists), summed preference credit and credited-entry count from the stored records of TDD-4.1.79, and a founder marker; island sections give the rater's like rate on population entries against random controls and against service picks, over the ratings up to the freeze with skips excluded, with the TDD-4.1.19 interval over digest weeks and the TDD-4.1.20 verdict (higher is favorable), and the migrations of TDD-3.1.73 admitted that week, recognized by a parent in another island. A comparison with fewer than two digest weeks of both origins has an unavailable interval and verdict. No column is computed from another. `web/report/` serves the report read-only under the automated-output notice. Test known fixtures per column, a genome with no rated entries (zero credit, zero count), and the q-bio report carrying zero preference columns with the stated reason.
...
| `GET /v1/snapshots/{id}/cards`, `/graph`, `/passages`, `/questions` | Tools receive scoped immutable paper cards, graph, passages and question definitions under scope `snapshots:read`, 404 to any other role, except that ingest also reads cards; no unscoped search for an agent |
| `GET /v1/snapshots/{id}/paper_manifest` | Ingest reads a sealed snapshot's frozen paper manifest hash, and with the pinned cards resolves each run's stamp without a database connection, under scope `snapshots:read`; 404 to any other role (#331) |
| `POST /v1/embedding-views` | Ingest records a published embedding view against the paper version the view names, so the day pass records its views without a database connection, under scope `embedding_views:record`; recording a view twice records it once; 403 to any other role (#331) |
| `GET /v1/islands/{island}/population` | Ingest reads one island's active and archived genomes, oldest admission first, each with its configuration hash, lineage, infrastructure hash, emphasis parts, founder flag and parent, so the day's issue selects genomes without a database connection, under scope `population:read`; the reader rebuilds each genome and refuses one whose parts do not hash to its configuration hash; 404 for an island outside the three and to any other role (#331) |
| `GET /v1/snapshots/{id}/members`, `/family`, `/overviews`, `/passage_index` | Tools list a snapshot's pinned members (family, version, card, overview, passage index and graph hashes), resolve a family to the one version it pins, and read pinned overview vectors and passage indexes by their pinned hash, under scope `snapshots:read` (#297, #287) |
| `GET /v1/snapshots/{id}/extraction?family_id=`, `/source?family_id=` | Tools read the extraction and the source document the pinned card of a family names by its recorded `extraction_hash` and `original_source`, the source as exact bytes; a card naming none, or an extraction of another version, is 422 `unavailable_input`; scope `snapshots:read` (#287) |
| `GET /v1/runs/{id}/specification` | Tools read the run's snapshot hash, allowed tools, paper, issued question ids and whether it is still active, under scope `runs:specification`, which does not widen `runs:read`; 404 for an unknown run and to any other role (#287) |
| `GET /v1/runs/{id}/worker` | A run worker, holding the orchestrator's certificate for its own reads and writes, loads the run's configuration id, attempt, genome hash, snapshot hash, budgets, allowed tools, paper, issued question ids and its genome's four emphasis parts (`prompt`, `scan_policy`, `read_policy`, `probability_assignment_rule`), under scope `runs:worker`, which does not widen `runs:read`; 404 for an unknown run and to any other role, 422 `unavailable_input` when the run's configuration has no stored genome (#306, #322) |
| `GET /v1/snapshots/{id}` | The orchestrator and tools read a sealed snapshot's hash, `sealed_at`, pinned family count and the sheet hashes its pins were made for, under scope `snapshots:read`; 404 for an unsealed snapshot and to any other role (#306) |
| `GET /v1/owner/papers/{id}`, `/v1/owner/runs/{id}` | The owner reads a paper family's page of runs with their endings and verdicts, its paper requests and the card record each run's snapshot pins for it, or one such run, under role `owner` and scope `owner:read`; any other role is refused 403 (#301) |
| `GET /v1/owner/runs?day=\|island=`, `/v1/owner/runs/{id}/settlement` | The owner lists the runs created on one UTC day or whose genome is on one island, oldest first from a cursor so a reader following new runs asks again from the last it saw, each with its genome's lineage and island, and reads one run's settlement, under role `owner` and scope `owner:read`; any other role is refused 403. `bin/tail-runs` follows the trace through these and the trace and run reads, one line per event (#326) |
...
| `POST /v1/runs/{id}/submit` | The tool service forwards a run's submit call, and the orchestrator keeps the operator path, scope `runs:submit`; storage rechecks slot, deadline, snapshot, retrieved evidence, uniqueness and budgets in one transaction; 409 once the run is void (#287) |
...
| `POST /v1/paper-requests`, `GET /v1/paper-requests`, `POST /v1/paper-requests/transitions` | Tool service records a run's request for an absent family (TDD-3.1.78); ingest lists the open ones and moves one to acquiring, acquired, failed or refused, storage refusing a start past the UTC day's acquisition budget in the same transaction; 404 on the read and 403 on either write to any other role |
| `GET /v1/ratings?rater_id=` | Inspector receives the digest entries one rater has rated (`entry_id`, `paper_hash`), never a rating's value; scope `ratings:rated`, 404 to any other role |
| `GET /v1/ratings?rater_id=&batch_id=` | Rating app receives one rater's own ratings of a batch's digest entries (`rating_id`, `digest_entry_id`, `paper_hash`, `value`, `rated_at`); scope `raters:read`, 404 to any other role (#252) |
| `GET /v1/preference?rater_id=&iso_week=` | Rating app receives one rater's stored preference credit of a week, one row per rating and genome (`rating_id`, `genome_hash`, `share`), never a total; scope `raters:read`, 404 to any other role (#252) |
...
| `POST /v1/settlements` | Orchestrator records the one settlement of an ended run, however it ended: `run_id`, `provider`, `model`, `input_tokens`, `output_tokens` and `usage_source` (`provider` when every response carried the provider's usage record, `loop_count` otherwise), from the `RunOutcome` that `agents/loop.py#run_conversation` hands its `settle` callable; `cost_micros` stays null until a price quote owner exists, a second settlement of a run is 409, and the row commits with its `run_settled` ledger event; 403 to any other role (#251) |
| `POST /v1/runs/{id}/resources` | Orchestrator records what a settled run cost the system, once: each model call's input, cached input and output tokens with the model identity the provider returned, its latency and bytes sent and received; the model endpoint and tool service it reached; its start, first and last model call and end instants; the process's CPU milliseconds and peak memory and host load at start and end; the worker image digest and run log path, null until a launcher reports them. Calls that do not sum to a settlement whose `usage_source` is `provider` are 409, as is a second record; an unsettled run is refused; the row commits with its `run_resources_recorded` ledger event; 403 to any other role. The owner's `GET /v1/runs/{id}/trace` carries it as `resources`, null until recorded, with the queue wait from the run's creation, each tool call's service time and the store paths of the trace's payloads; `bin/tail-runs` prints it at the run's end and `bin/swarm` on `i`. Nothing of it reaches the agent (#330) |
| `GET /v1/owner/costs?day=` | Owner receives settled spend of one UTC day and of its month up to that day's end, per island and per configuration: `cost_micros` summed over priced rows, unpriced rows counted with their tokens and never priced; scope `owner:read`, 403 to any other role (#251) |
...
Scheduler creates one population slot per active configuration of the paper's island per sampled paper before dispatch. No comparison slot exists before the RD-23 registration (decision 0024, #238); once it is recorded, two separate with/without-Jev evidence-first comparison slots are created per eligible study paper, slot identity includes arm so comparison runs cannot overwrite population runs, and every slot consumes the same global concurrency/spend limits. Comparison runs do not nominate into the population digest and are not reused as population runs. A pair's assigned arm is fixed before requests; preserve missing/failed arm outcomes. Capacity qualification covers the whole slot set at the current population size, not one fixed count.
...
| `StorageHandlers.get_snapshots_id_paper_manifest` | GET /v1/snapshots/{id}/paper_manifest (200) | no arguments | `{snapshot_id:Hash,paper_manifest_hash:Hash}` |
| `StorageHandlers.post_embedding_views` | POST /v1/embedding-views (200) | `Command<{view_hash:Hash}>` | `{view_hash:Hash}` |
| `StorageHandlers.get_islands_island_population` | GET /v1/islands/{island}/population (200) | no arguments | `{island:"cs"\|"quant-ph"\|"q-bio",genomes:List<{configuration_hash:Hash,lineage_id:string,infra_hash:Hash,emphasis:Map<string, string>,founder:bool,parent_hash:Hash\|null,archived:bool}>}`; active genomes, then archived, each oldest admission first |
...
| `StorageHandlers.post_runs_id_submit` | POST /v1/runs/{id}/submit (200) | `Command<{run_id:Id,submission_id:Id,answers,nomination}>`; `run_id` equals the route id; answers and nomination as TDD-3.1.57 | accepted: `{accepted:true,run_id:Id,submission_id:Id,receipt:CommitReceipt}`, an identical retry of an already accepted submission carrying `receipt:{replay:true,accepted_at:Utc}`; rejected attempt: `{accepted:false,reason:Text[1..512],receipt:CommitReceipt}` (#293) |
...
| `StorageHandlers.get_runs_id_specification` | GET /v1/runs/{id}/specification (200) | no body, no query; `tools` role, scope `runs:specification` | `{run_id:Id,snapshot_hash:Sha256,allowed_tools:List<ToolName>,paper_id:Text,issued_question_ids:List<Id>,active:bool}`; `active` is false once the run holds a terminal state (#287) |
| `StorageHandlers.get_runs_id_worker` | GET /v1/runs/{id}/worker (200) | no body, no query; `orchestrator` role, scope `runs:worker` | `{run_id:Id,configuration_id:Id,attempt:NonNegative,genome_hash:Sha256,snapshot_hash:Sha256,budgets:RunBudgets,allowed_tools:List<ToolName>,paper_id:Text,issued_question_ids:List<Id>,prompt:Text,scan_policy:Text,read_policy:Text,probability_assignment_rule:Text}`; the run's fields exactly as stored, each emphasis part the `genome_parts` row of that name for the run's `configuration_id`; 422 `unavailable_input` when any is missing (#306, #322) |
| `StorageHandlers.get_snapshots_id` | GET /v1/snapshots/{id} (200) | no body, no query; `orchestrator` or `tools` role, scope `snapshots:read` | `{snapshot_hash:Sha256,sealed_at:Utc,pinned_family_count:NonNegative,sheet_hashes:List<Sha256>}`; distinct pinned paper families, sheet hashes in hash order; 404 when no such snapshot is sealed (#306) |
| `StorageHandlers.get_runs` | GET /v1/runs (200) | query exactly one of `configuration_id:Id`, `batch_id:Sha256` or `paper_id:Text[1..128]`; `cursor:string\|absent`; no other argument | `{runs:List<RunRecord>[0..50],next_cursor:string\|null}`; newest first, fixed page size; a batch or paper with no runs is an empty page, not 404 (#128, #253) |
| `StorageHandlers.get_sheets_hash` | GET /v1/sheets/{hash} (200) | no body, no query; `hash` is the sealed sheet hash a run's `batch_id` carries | `{sheet_hash:Sha256,sealed_at:Utc,questions:List<{question_id,target_definition_hash,resolver_id,resolver_version,horizon}>}`; questions in sealed order, exactly as stored; 404 when no such sheet is sealed (#253) |
...
| `StorageHandlers.get_configurations` | GET /v1/configurations (200) | query `cursor:string\|absent`; no other argument | `{configurations:List<GenomeRecord>[0..50],next_cursor:string\|null}`; every admitted genome, run or not, newest admission first, fixed page size (#177) |
| `StorageHandlers.get_configurations_id` | GET /v1/configurations/{id} (200) | no body, no query; `id` is the configuration id `runs.configuration_id` carries | `GenomeRecord`: `{configuration_id:Id,configuration_hash:Hash,lineage_id:string,island:Island,founder:bool,infra_hash:Hash,parent_hash:Hash\|null,parts:List<{part,value,value_hash}>[4],admission:{disposition:"seeded"\|"accepted",profile_hash:Hash},admitted_at:Utc,archive:{cycle_id,skill,resolved_claim_count,profile_hash,archived_at}\|null}`; 404 when the population store holds no such genome (#177) |
| `StorageHandlers.get_configurations_id_forecasts` | GET /v1/configurations/{id}/forecasts (200) | query `cursor:string\|absent`; no other argument | `{forecasts:List<{submission_id,run_id,question_id,confidence,horizon,sealed_at,resolver_id,resolver_version,resolution:{resolution_id,status,resolver_id,resolver_build_digest,resolution_version,resolved_at}\|null}>[0..50],next_cursor:string\|null}`; sealed claims of the configuration's runs, newest seal first, each with its latest resolution or null while pending (#177) |
| `StorageHandlers.get_owner_papers_id` | GET /v1/owner/papers/{id} (200) | `id` is the paper family id; query `cursor:string\|absent`, no other argument; `owner` role, scope `owner:read` | `{paper_id:Id,runs:List<OwnerRun>[0..50],next_cursor:string\|null,requests:List<{request_id,run_id,snapshot_hash,status,reason,requested_at,started_at,paper_version_id}>,cards:List<{snapshot_hash,paper_version_id,card_hash,card:object}>}`; runs that read the family newest first, requests oldest first, and for each snapshot a run on the page received the card record it pins for the family, exactly as stored; 404 when no run, request or snapshot pin names the family; 422 `unavailable_input` when a pinned card record is unreadable (#301) |
| `StorageHandlers.get_owner_runs_id` | GET /v1/owner/runs/{id} (200) | no body, no query; `owner` role, scope `owner:read` | `OwnerRun`: the run record as stored with `events` (its `run_events` by hash in order), `ending:{state,reason,ended_at,submission:{submission_id,accepted_at,forecasts:List<{question_id,probability,rationale,evidence_ids}>,nomination}\|null}\|null` and `outcomes` (its sealed claims, each with its latest resolution, as the configuration's forecasts read carries them); 404 for an unknown run (#301) |
| `StorageHandlers.get_owner_runs` | GET /v1/owner/runs (200) | query exactly one of `day:UtcDate` or `island:Island`; `since:Utc\|absent`; `cursor:string\|absent`; each at most once, no other argument; `owner` role, scope `owner:read` | `{runs:List<RunRecord & {lineage_id:string\|null,island:Island\|null}>[0..50],next_cursor:string\|null}`; runs created that day, or of the island's genomes, at or after `since` and after `cursor`, oldest first, fixed page size; `lineage_id` and `island` null for a configuration the population store does not hold; a selection with no runs is an empty page (#326) |
| `StorageHandlers.get_owner_runs_id_settlement` | GET /v1/owner/runs/{id}/settlement (200) | no body, no query; `owner` role, scope `owner:read` | `{run_id:Id,provider:Text,model:Text,input_tokens:NonNegative,output_tokens:NonNegative,usage_source:"provider"\|"loop_count",cost_micros:NonNegative\|null,settled_at:Utc}`, the run's `run_settlements` row as stored; 404 for a run not held or not yet settled (#326) |
...
The former `/snapshots/{id}/...` is only the four enumerated routes above and the member, extraction and source reads of the storage-owned route list; `GET /v1/snapshots/{id}` itself describes the sealed snapshot (#306). Search, neighbors and deep-read pagination are tool/reader computations over authorized snapshot artifacts, not an extra unscoped storage API. Snapshot route callers authenticate scope with `X-Run-Id: Id` when serving a run; storage independently binds it to the stored run/snapshot. A reader batch principal instead presents its fenced job id/epoch and is restricted to that job's declared inputs.

The eight `inspector`-scoped read routes (`get_runs_id`, `get_runs`, `get_submissions`, `get_manifests_hash`, `get_configurations`, `get_configurations_id`, `get_configurations_id_forecasts`, `get_sheets_hash`) return only what storage already holds for runs, run events, submissions, manifest artifacts, sealed sheets, the population store and resolutions, exactly as stored; each refuses every role but `inspector`, the three configuration routes need the `configurations:read` or `forecasts:read` scope, and `get_sheets_hash` needs `forecasts:read`, since a sheet's questions are what its forecasts answer. `get_runs` by `batch_id` uses the `(batch_id, paper_id, configuration_id, attempt)` unique key; by `paper_id` (decision 0022: one paper per run) it scans `runs`, which has no index on that column yet. The population store is the `genomes`, `genome_parts` and `genome_archive` tables, written only through `evolution/population.py#PopulationStore`: a seeded genome or an accepted child (AG-21) with its four hashed emphasis parts (AG-20) and admission record in one transaction, and a select stage's archive entries (FT-15) in one transaction, each appending one `genome_admitted` or `genome_archived` ledger event and nothing on an identical replay. A forecast's verdict is its latest `resolutions` row. No scorer output is stored yet, so no route returns a Brier contribution and none computes one. `get_manifests_hash` resolves its path hash the same way `artifact_in_job_scope` and `ArtifactVerifier` do: as an `artifact_productions.manifest_hash`, not a payload's own `sha256`, so the same hash a run's `model_identity.agent_model_manifest` carries can be pasted into the route directly.

<a id="storage-contracts-binary-artifact-protocol"></a>
#### Binary artifact protocol

POST uses `Content-Type: multipart/form-data` with exactly two parts: `metadata` (`application/json`, at most 128 KiB, `ArtifactUpload`) and `payload` (`application/octet-stream`, bytes). Never embed base64 PDFs/weights/vectors in JSON. `ArtifactUpload={schema_version:1,command_id:Id,request_id:Id,expected_hash:Hash,byte_length:Count,media_type:"application/json"|"application/pdf"|"application/octet-stream"|"image/png"|"text/plain",kind:ArtifactKind,input_hashes:List<Hash>[0..1000],producer_version:ProducerVersion,config_hash:Hash,source_available_at:Utc|null,retention_policy_hash:Hash}`. `ArtifactKind` = source_response, source_document, extraction, vector_payload, model_weights, tokenizer, manifest, tool_request, tool_response, provider_response, study_evidence, signature_evidence. Storage also holds the kinds trace_request and trace_response, written only by the trace commands of TDD-2.1.2 and never admitted by this upload. A model_weights binary can be up to 128 GiB, other binaries up to 1 GiB; actual admitted job/source cap may be lower. Reject declared or streamed overage; stream without buffering whole body. Storage assigns `created_at` after fsync; validates any source_available_at against authenticated capture evidence; actual runtime publication comes from ArtifactPublicationReceipt. Input hashes must all be committed/visible. ArtifactReceipt is `{artifact_hash:Hash,byte_length:Count,created_at:Utc,receipt:CommitReceipt}`. GET sets exact Content-Length, Content-Type and `ETag: "<hash>"`; no content transformations. Error replies are the JSON Reply error union. Missing, tombstoned and unauthorized hashes return indistinguishable 404 not_found to callers without tombstone-audit privilege.
...
5. Serialization/deadlock retry at most six attempts; the wait before each retry is drawn uniformly from zero to an exponential ceiling that starts at 10 ms and doubles, capped at 500 ms (full jitter), preserving command content and no external call inside transaction. Exhaustion returns 503, retryable. Lost connection after COMMIT is resolved by replay, never guessing failure.
...
| preference_credits | rating_id,genome_hash,island,entry_id,sealed_probability,share,iso_week; PK(rating_id,genome_hash) | FK rating_id, genome_hash references genomes(configuration_hash), entry_id; share is signed and nonzero; immutable; read by no scorer or resolver path (#140) |
| preference_credit_gaps | rating_id PK,iso_week,reason,recorded_at | FK rating_id; a population rating whose nominators could not be read, credited to no genome; immutable (#140) |
...
| `PaperVersionRecord` | `family_id: PaperFamilyId`, `version_id: PaperVersionId`, `external_ids: ExternalIdentifier[1..64]`, `is_first_public_version: bool`, `first_public_at: UtcInstant\|null`, `first_public_interval: SourceInterval\|null`, `first_public_evidence_hashes: Sha256[1..64]`, `source_access_hashes: Sha256[1..64]`, `title: string`, `abstract: string`, `author_ids: string[]`, `primary_category: string\|null`, `primary_source_subfield: string\|null`, `original_source_hash: Sha256`, `text_source_kind: latex\|pdf\|metadata`, `source_revision: string`, `author_count: Count`, `categories: string[1..unbounded]`, `version_count: PositiveCount`, `created_at: UtcInstant`. Exactly one of first_public_at and first_public_interval is nonnull. Uncertain first-public time blocks head training/forecast eligibility until resolved; do not fabricate t0. Later revisions preserve the family and get new version/source identities. `categories` is ordered, primary first, read from the arXiv listing record; `author_count` and `version_count` come from the same record (#149). |
...
| `TargetRegistry` | `protocol: "automatic-citations-v1"`, `definitions: TargetDefinition[3]`, `calibrated_domains: ["cs.AI","cs.LG","quant-ph","q-bio"]`, `created_at: UtcInstant`; order fixed, unique ids, full definition body hash bound into questions/bundles/labels. Domain ids are the primary categories calibration is computed within, not claims of generalization. |
...
| Record | Exact fields and invariants |
| --- | --- |
| `CorpusRow` | `paper_family_id: PaperFamilyId`, `original_version_id: PaperVersionId`, `t0: UtcInstant\|null`, `publication_week: string\|null`, `primary_category: string\|null`, `source_subfield: string\|null`, `selection_rank: Count`, `feature_hash: Sha256\|null`, `label_hashes: (Sha256\|null)[3]`, `known_mask: bool[3]`, `partition: pilot\|fit\|development\|calibration\|locked_evaluation\|refresh_fit\|refresh_calibration\|excluded`, `exclusion_reasons: (shortfall\|unknown_t0\|family_alias\|source_unavailable\|feature_unavailable\|missing_label\|pilot_reserved\|consumed_holdout\|slice_unqualified)[]`, `author_count: Count\|null`, `categories: string[1..unbounded]\|null`, `version_count: PositiveCount\|null`. Known masks equal resolved true/false labels. Missing label need not exclude other prediction heads. Week is ISO YYYY-Www from t0 UTC. The three declared metadata fields carry the same `PaperVersionRecord` values a historical family's card would for `CardMetadata`; null exactly when no paper resolved for the row (#149). |
...
| `CardMetadata` | `author_count: Count`, `categories: string[1..unbounded]`, `abstract_tokens: Count`, `title_tokens: Count`, `first_available_weekday: 0..6`, `code_link: bool`, `version_count: PositiveCount`. Every field is available at seal time from the paper card and nothing else; categories ordered, primary first, and the primary must name an admitted category. No API type; the source record `assemble_metadata_block` reads to build one row's metadata block (#149 Appendix B). |
| `Standardization` | `mean: Finite[17]`, `std: Finite[17]`. Fit once on the fitting partition's raw metadata rows and stored on the head bundle; a one-hot or flag column keeps the fixed identity transform (mean 0, std 1) rather than a fitted one; std strictly positive. Calibration, promotion and inference all standardize through the exact stored values the fit used (#149). |
| `TrainingArrays` | `ordered_family_ids: PaperFamilyId[1..unbounded]`, `features: TensorRef`, `labels: TensorRef`, `known_mask: TensorRef`, `feature_hashes: Sha256[1..unbounded]`, `label_hashes: (Sha256\|null)[3][]`, `corpus_release_hash: Sha256`, `split_hash: Sha256`, `target_registry_hash: Sha256`, `representation_hash: Sha256`, `partition: fit\|development\|calibration\|locked_evaluation\|refresh_fit\|refresh_calibration`. N is row count; features float32 [N,1553], the [N,1536] embedding block followed by the [N,17] declared metadata block (#149 Appendix B); labels/mask uint8 [N,3]. Unknown entries have stored Y=0 and M=0 as a serialization placeholder only; they never enter class counts/loss. Row arrays have equal N, features complete and same identity. Empty partitions return an explicit insufficient-support result, not a zero-dimensional tensor. |
...
| `LinearHead` | `target_id: TargetId`, `target_definition_hash: Sha256`, `weights: TensorRef`, `intercept: Finite`, `lambda: Finite`, `fit_diagnostics: FitDiagnostics`, `candidate_diagnostics: FitDiagnostics[5]`, `fit_row_ids_hash: Sha256`, `development_row_ids_hash: Sha256`, `standardization: Standardization`. Weights float64 [1553]; lambda in {0.0001,0.001,0.01,0.1,1}; no intercept penalty. At least one candidate converges. A bundle without a stored standardization is refused (#149). |
| `SigmoidCalibrator` | `target_id: TargetId`, `primary_category: string`, `a: Finite`, `b: Finite`, `penalty: 0.000001`, `solver: "L-BFGS-B"`, `iterations: Count`, `projected_gradient_inf_norm: Finite`, `converged: bool`, `calibration_row_ids_hash: Sha256`, `positive_count: Count`, `negative_count: Count`, `solver_runtime_hash: Sha256`. a>=0. No calibration/development row enters coefficient fitting. |
| `TargetBundleEntry` | `target_id: TargetId`, `target_definition_hash: Sha256`, `status: qualified\|unavailable`, `head_hash: Sha256\|null`, `calibrator_hashes: Map<string, Sha256>\|null`, `qualification_report_hash: Sha256\|null`, `fitting_base_rates: Map<string, Probability>\|null`, `unavailable_reason: insufficient_coverage\|insufficient_classes\|fit_failed\|calibration_failed\|failed_skill\|invalidated\|not_fitted\|null`. Qualified requires the head hash, the report hash, one calibrator and one base rate per served primary category, and null reason; a category absent from both maps is unavailable for that target. |
| `ModelBundle` | `target_registry_hash: Sha256`, `representation_hash: Sha256`, `feature_policy: "overview_passage_sqrt2_v1"`, `dimension: 1553`, `targets: TargetBundleEntry[3]`, `corpus_release_hash: Sha256`, `split_hash: Sha256`, `fitting_cutoff: UtcInstant`, `runtime_manifest_hash: Sha256`, `previous_bundle_hash: Sha256\|null`, `created_at: UtcInstant`. Order equals registry, all label availability <= cutoff. Bundle availability is not all-three qualification. |
...
| `PaperCardBody` | `schema_version: 1`, `paper_family_id: PaperFamilyId`, `paper_version_id: PaperVersionId`, `as_of: UtcInstant`, `overview: CardOverview`, `first_public_at: UtcInstant\|null`, `original_source: SourceLocator`, `overview_available: bool`, `passage_coverage: complete\|partial\|unavailable`, `passage_count: Count`, `sections: CardSection[0..40]`, `unlisted_section_count: Count`, `extraction_hash: Sha256\|null`, `representation_hash: Sha256\|null`, `head_feature_eligible: bool`, `head_feature_unavailable_reason: string\|null`, `head_predictions: HeadCardValue[3]`, `neighbors: NeighborCardSummary[0..5]`, `neighbor_embedding_distance: AvailabilityValue`, `neighbor_outcomes: NeighborTargetValue[3]`, `graph: GraphCardValues`, `author_citations: AuthorCitationValue[]`, `jev: JevCardAssessment`, `card_token_count: Count`, `author_count: Count`, `categories: string[1..unbounded]`, `version_count: PositiveCount`, `title_tokens: Count`, `abstract_tokens: Count`, `code_link: bool`, `first_available_weekday: 0..6\|null`. Paper-card token count <=3000 selected embedding tokens; no extra popularity/social-counter fetch to fill omissions. Author/reference/evidence detail may be reached by existing bounded tools; the builder must not omit required three-head states or silently trim string values. Oversized source overview uses the referenced-overview union, preserving identity, every availability field and provenance; it never makes the whole paper unavailable. Validate the fixed non-overview projection fits the cap before reader activation. Source detail projection and its complete artifact references must satisfy this exact schema; no field may be silently removed to achieve the cap. The declared metadata fields feed `CardMetadata` for prediction-head fitting (#149 Appendix B); `first_available_weekday` is derived from `first_public_at` and is null exactly when it is. |
| `CardSection` | `title: string`, `passage_count: PositiveCount`, `first_passage: PositiveCount`, `last_passage: PositiveCount`. One top-level section title of the paper's passage records; the range numbers the paper's passages from 1 in document order and holds exactly `passage_count` passages. A card's sections tile that numbering from 1 without gaps, never past `passage_count`; `unlisted_section_count` is nonzero only when 40 are listed. Assembled from the passage records the build declares, which belong to the card's paper version and number `passage_count`; fewer than two top-level sections, or no passage records, assemble no sections (#270). |
...
Field and category ids below are stable implementation spellings of the RD-16 rows, not new scientific labels. A field id names one primitive and one scale in every rubric version that asks it. `jev-rubric-v2`, the launch rubric, asks in order:

| Field id | Primitive | Options or scale |
| --- | --- | --- |
| `primary_contribution` | `choice` | its category enum below |
| `evaluation_rigor` | `score` | points 0 to 4 |
| `limitations_candor` | `score` | points 0 to 3 |
| `novelty_as_claimed` | `score` | points 0 to 3 |
| `claims_supported_by_evidence` | `noul` | probability in [0,1] |
| `reproducible_from_materials` | `noul` | probability in [0,1] |
| `generalizes_beyond_main_setting` | `noul` | probability in [0,1] |
| `open_problems_stated` | `noul` | probability in [0,1] |

`jev-rubric-v1` asks the eight `choice` fields below, in this order. Each has exactly its own enum and no category is silently borrowed from another field.
...
`JevFieldId` is the union of the field ids above. `JevCategory` is the union of category literals above, but every record validates membership in the enum for its field; the union is never sufficient validation. `JevFieldMap<T>` means an object with exactly the eight named keys of one rubric version, each with its field-specific typed T; the key set names the version, and a card that names its rubric version must carry that version's keys. It is not a string-keyed extensible dictionary. `JevFieldValue` is `JevFieldResult|JevScoreResult|JevNoulResult`, matched to the field's primitive; a stored `JevFieldResult` carries no `type` key, as v1 stored it, and the other two carry theirs. Category probabilities use ordered entries so duplicate keys cannot silently overwrite.
...
| `ScoreQuestion` | `type: "score"`, `field_id: JevFieldId`, `question: string`, `criteria: string[n]`, one nonempty criterion per scale point of the field, point 0 first. |
| `NoulQuestion` | `type: "noul"`, `field_id: JevFieldId`, `question: string`, `statement: string`. The question states the statement; the answer is the probability it is true. |
| `JevRubric` | `version: jev-rubric-v1\|jev-rubric-v2`, `questions: (RubricQuestion\|ScoreQuestion\|NoulQuestion)[8]`, `created_at: UtcInstant`. Exactly the version's eight fields in fixed order, each in its primitive; full criteria match RD-16. No quality or future-impact question; novelty only as the paper claims it. |
...
| `JevFieldResult` | `field_id: JevFieldId`, `selected_category: JevCategory`, `distribution: CategoryProbability[4..8]`, `provider_confidence: Probability\|null`, null only when the verified interface returns no confidence. Distribution contains all and only that field's categories and sums to one within the provider's two-decimal rounding, 0.05. Selected category must be in registry; preserve provider choice rather than silently recomputing argmax. Confidence is not measured accuracy and has no cutoff. |
| `JevScoreResult` | `type: "score"`, `field_id: JevFieldId`, `score: number`, `legend: string[n]`, `distribution: Probability[n]`, `provider_confidence: Probability\|null`. `n` is the field's scale; the score is the provider's probability-weighted position on it, from 0 to n-1, and the most probable point is derived from the distribution; the legend is the question's criteria as the provider returned them; the distribution is in point order and sums to one within 0.05. |
| `JevNoulResult` | `type: "noul"`, `field_id: JevFieldId`, `probability: Probability`, `statement: string`. The probability that the question's statement is true; the statement is copied from the question. No confidence is returned. |
| `JevAvailable` | `status: "available"`, `fields: JevFieldMap<JevFieldValue>`, `input_hash: Sha256`, `rubric_hash: Sha256`, `provider_identity: JevProviderIdentity`, `sanitized_request_hash: Sha256`, `sanitized_response_hash: Sha256`, `computed_at: UtcInstant`, `smoke_report_hash: Sha256\|null`. Field_id must equal map key. Null is permitted only for engineering or smoke-test processing artifacts and cannot enter a study paper card; the smoke test can therefore run before its report exists. |
...
| `JevCardAvailable` | `status: "available"`, `assessment_id: Sha256`, `fields: JevFieldMap<JevFieldValue>`, `paper_version_id: PaperVersionId`, `extraction_hash: Sha256`, `rubric_hash: Sha256`, `rubric_version: string`, `provider_identity: JevProviderIdentity`, `computed_at: UtcInstant`, `qualification_report_hash: Sha256`. Projection from a stored valid JevAvailable with matching qualification; input version/extraction must equal the card-selected paper, all artifacts available by `card.as_of`. No sanitized-request/response hashes or billing fields are exposed. The fields are exactly the eight of `rubric_version`. Its rendered text shows per field only the value and what makes it readable (a choice's option and distribution, a score's legend line and distribution, a noul's probability and statement) with provider confidence, and no hash, instant, identity kind or report id. |
...
| `InspectorHandlers.get_agents` | GET `/agents` | query `cursor:string\|absent` | HTML table of the population (`get_configurations`), newest admission first, each linking to its agent page, including a configuration that has never run; a `next` link when a further page exists |
| `InspectorHandlers.get_agents_configuration_id` | GET `/agents/{configuration_id}` | UUIDv4 path; query `cursor:string\|absent`, `forecast_cursor:string\|absent` | HTML genome section (hashed parts, island, lineage, admission record, archive), the configuration's runs newest first, and its verdict section; each listing has its own `next` link when a further page exists |
...
Every page reads storage only through `StorageClient` (PL-02 by way of the client's typed request/response handling; `contracts/http.py#ServiceContract` does not exist yet, so this is a documented narrowing of PL-02, not the declared interface it eventually asks for). No page recomputes a stored field, fetches paper content, or serves a route beyond the four enumerated above and their `/api/v1` twins (Web JSON contract).

<a id="service-api-owner-actions-application"></a>
### Weekly island report application

Read-only server-rendered HTML (`web/report/`), one page per island and ISO week at `/reports/{island}/{iso_week}`, under the automated-output notice; it has no form, no write route and no route that asks for a replacement or a parent draw. The page is built from a report the weekly cycle's report step loads through `PreferenceRepository` (`read_week_population`, `read_credits`, `read_rated_entries`) and `measurement/weekly.py#island_report` (#140). Its genome table has one row per genome present at the freeze with a founder marker, one skill column per registered target, the summed preference credit and the credited-entry count as separate columns; an island section gives the like rate of population entries against random controls and against service picks with the interval and verdict, and lists the migrations admitted that week. The q-bio page states that the island has no rater and shows zero preference. A field migration keeps a local parent and no stored record marks it, so the list holds the migrations that admit another island's genome as the parent. An unknown island, a malformed week or a week with no report is a 404. The page's JSON twin is `/api/v1/reports/{island}/{iso_week}` (Web JSON contract).

### Owner actions application

Server-rendered HTML over one owner session (`web/actions/`), separate from the read-only inspector and from the rating app: the session cookie is `owner_session`, never `rater_session`, a rater credential does not authenticate here, and a session expires 24 hours after creation with server-side revocation as for a rater (Private rating application). Every mutating route requires the session's CSRF token, refusing a missing or wrong token with 403. There is no registration: an operator provisions the one owner principal (`owner_principals`, a salted PBKDF2 credential hash, ledger kind `owner_provisioned`), and the owner identity every command records comes from the session, never from a form field. The app reaches storage only through a `StorageClient` over the same mutually authenticated boundary the rating and inspector apps use, and imports no storage repository module. Storage serves `storage/actions.py#OwnerActions` under the `owner` role: `POST /v1/owner/admit`, `/v1/owner/seed` and `/v1/owner/retire` need the scopes `owner:admit`, `owner:seed` and `owner:retire` and answer 403 to any other role or a missing scope, and the reads `GET /v1/owner/genomes/{configuration_id}`, `/v1/owner/admissions/{configuration_id}`, `/v1/owner/retirements/{configuration_id}` and `/v1/owner/retrospective` need `owner:read` and answer 404 otherwise, as the `inspector` reads do. A command body that is not exactly its closed shape is refused 422 before any write, and a refusal named below is a 200 result carrying its reason, which the app turns into the status listed for it.
...
An admission also writes a `genome_owner_admissions` row, ledger kind `genome_owner_admission_recorded`, naming the owner, the time, `kind` (`edit` naming its source, or `seed`); an edit writes the genome, its `genome_admitted` event, this row and its event in one transaction, so an admitted edit always has its attribution and a failure between them leaves neither; a seed writes the genome first and this row after, so a failure between the two leaves an unattributed genome, never an attribution without a genome. An identity that is not a provisioned owner is refused by every command before any write (`not_owner`, 403). Refusals answer 409 with the reason as the detail, 404 for an unknown genome or template, 422 for a malformed form value.

`GET /api/v1/health` serves the platform health monitor (`platform/health.py#HealthMonitor.report`, TDD-2.1.33) to the owner session only, and answers 401 without one, as every route here does. The body is `{state, checked_at, checks[{name, state, detail}]}`: one check per supervised service in the monitor's four states, and an overall `state` that is the most severe of them (`operator_repair`, then `failed`, `waiting`, `healthy`), or `waiting` when no service has been observed. The route reads the report from a provider its composer supplies and answers 503 when none is wired, never a made-up state (#254). The one-line state a rater sees is the rating app's concern and not served here.

A browser cannot present the certificate the inspector app authenticates by, so this app also serves the inspector's reads under the owner session (#255): the population page, the run page and the manifest page render the same views as the inspector's (`web/inspect/views.py`), and the agent page merges the inspector's runs and verdict sections with the owner's genome view, admission history, retirement status and forms. They are read through a second `StorageClient` holding the `inspector` role's read scopes and `ratings:rated`, so the owner's writes and the inspector's reads stay under their own roles; without that client these routes answer 503 and the agent page omits the two sections. The inspector app itself is unchanged.

`GET /api/v1/digests/{hash}` serves a stored digest (`GET /v1/digests/{hash}`) to the owner session, who is also a rater, with the rating guard of SR-25 applied: an entry the owner's rater identity has not rated carries only `entry_id`, `paper_hash`, `display_position` and `rated: false`, with no origin, service source, control provenance or nomination, and an entry it has rated carries the full record and `rated: true`. The rated set is `GET /v1/ratings?rater_id=` (the inspector role, scope `ratings:rated`, 404 to any other role or a missing scope), which returns each rated entry's `entry_id` and `paper_hash` and never the rating's value. The route answers 404 for an unknown digest and 503 when no rater identity is configured for the owner. The agents' chance and agreement figure that the guard is meant to withhold is not yet defined, so no such figure is served or computed here; it is an open point on #255.

`GET /api/v1/owner/papers/{paper_id}` shows the owner everything the agents saw of one paper family, composed from stored records and storing nothing (#301): the path of its embedding view, whether a stored paper request acquired it and every request with the run that asked, the card record each run's snapshot pins for it, exactly as `query_cards` returns it to a run, and a page of the runs that read it, each with its model turns by hash, its accepted submission or void reason, its sealed claims' latest verdicts and the path of its trace. `GET /api/v1/owner/runs/{run_id}/trace` returns the same run with every tool call in call order: its decision and refusal reason, the request and the envelope the run received as stored text beside the artifact holding their exact bytes, and the `deep_reads`, `images` and `tool_calls` left after it. Both read through the owner's `StorageClient` and answer 404 for a family or run storage holds nothing about. The rating app serves neither.
...
| `ActionsHandlers.get_api_v1_health` | GET `/api/v1/health` | session | envelope `data` `{state, checked_at, checks[{name, state, detail}]}`; 503 when no monitor is wired |
| `ActionsHandlers.get_agents` | GET `/agents` | query `cursor:string\|absent`; session | the inspector's population table; 422 for a malformed cursor, 503 without the inspector client |
| `ActionsHandlers.get_runs_run_id` | GET `/runs/{run_id}` | UUIDv4 path; session | the inspector's run page; 404 for an unknown run |
| `ActionsHandlers.get_models_manifest_hash` | GET `/models/{manifest_hash}` | Sha256 path; session | the inspector's manifest page; 404 for an unknown hash |
| `ActionsHandlers.get_api_v1_owner_papers_paper_id` | GET `/api/v1/owner/papers/{paper_id}` | UUIDv4 path; query `cursor:string\|absent`; session | envelope `data` per `owner-paper.json`; 404 for an unknown family, 422 for a malformed cursor |
| `ActionsHandlers.get_api_v1_owner_runs_run_id_trace` | GET `/api/v1/owner/runs/{run_id}/trace` | UUIDv4 path; session | envelope `data` per `owner-run-trace.json`; 404 for an unknown run |
| `ActionsHandlers.get_api_v1_digests_hash` | GET `/api/v1/digests/{hash}` | Sha256 path; session | envelope `data` digest with unrated entries reduced to `entry_id`, `paper_hash`, `display_position` and `rated: false`; 404 for an unknown digest, 503 without the inspector client or the owner's rater identity |
| `ActionsHandlers.get_agents_configuration_id` | GET `/agents/{configuration_id}` | UUIDv4 path; query `cursor:string\|absent`, `forecast_cursor:string\|absent`; session | HTML genome, its owner history, the inspector's runs and verdicts, the edit form and the retire control; 404 for a configuration the population store does not hold |
...
Every HTML route of the rating, inspector, weekly report and owner actions apps has a JSON twin under `/api/v1/` in the same app, built from the same view the page renders and the same command the form runs (`web/api.py`, #249); `GET /api/v1/health`, `GET /api/v1/digests/{hash}`, the owner's paper page and run trace reads (Owner actions application, #301) and the owner's `GET /api/v1/costs?day=` are JSON only, the last reporting settled spend of a UTC day and its month against the profile's caps, `funded`, `paid_execution_enabled` and the USD 2 Jev daily sublimit labelled `profile_constant` (#251), as are the rating app's `GET /api/v1/ratings?batch_id=` and `GET /api/v1/credit?week=`, which read the session rater's own ratings with `value` and `rated_at` and their preference credit as a share per rating, never a total, and refuse 403 a `rater_id` query naming another rater (#252). `docs/contracts/api-v1/endpoints.json` is the endpoint table (app, method, path, the HTML route it twins, success status, schema) and the JSON Schemas beside it are the objects; the rating digest's twin is `GET /api/v1/digest`, the owner retrospective's `GET /api/v1/retrospective`. Success is `{contract: "1", data}`; a refusal is `{contract: "1", error: {code, message, field}}` with `code` fixed by status (400 and 422 `invalid_request`, 401 `unauthenticated`, 403 `forbidden`, 404 `not_found`, 409 `state_conflict`, 502 `not_saved`, 503 `unavailable`), an owner command's refusal reason as its `message`, and `field` naming the request field or header at fault or null. Every array directly under `data` is `{items, next_cursor}`, the cursor passed back unchanged; instants are the one UTC instant format; money is integer micros. A field a view withholds is omitted, never null: a rater's digest entry is closed to `paper_hash`, `digest_entry_id`, `title` and `abstract` (SR-21, SR-22, SR-25), and the owner's agent view has no `inspected` field without the inspector client. Every POST with a session carries `X-CSRF-Token`, refused 403 when missing or wrong, and `Idempotency-Key`, refused 400 when missing: the same key with the same request returns the first answer marked `X-Replayed: true` and changes nothing, the same key with another request is 409, keys are scoped to the session's principal, and a rating's key also derives the storage command and idempotency ids, so storage replays a retry after a restart. `POST /api/v1/login` has no session and carries neither header, as the HTML sign-in form carries no CSRF token. A POST body is a closed object of string fields, an unknown, missing or non-string field refused 422. A v1 field is never renamed or removed; a breaking change is `/api/v2`. `tests/contracts/test_api_v1.py` checks that every served `/api/v1` route is in the table and every HTML route has its twin; `tests/web/test_api_v1_*.py` validate a real response of every endpoint against its schema and refuse a field the page renders that the JSON lacks.
