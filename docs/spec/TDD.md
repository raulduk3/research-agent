...
| `JevAvailable` | `status: "available"`, `fields: JevFieldMap<JevFieldResult>`, `input_hash: Sha256`, `rubric_hash: Sha256`, `provider_identity: JevProviderIdentity`, `sanitized_request_hash: Sha256`, `sanitized_response_hash: Sha256`, `computed_at: UtcInstant`, `smoke_report_hash: Sha256\|null`. Field_id must equal map key. Null is permitted only for engineering or smoke-test processing artifacts and cannot enter a study card; the smoke test can therefore run before its report exists. |
...
| `JevCardAssessment` | `assessment: JevCardAvailable\|JevCardUnavailable`, `source_label: "Jev paper-content assessment, unqualified: not measured against human labels"`. Agent-only projection; human rating views use their separate allowlisted schema, not this artifact wholesale. |
...
External provider payloads are fixed by the preserved provider capability and retention evidence before activation; the adapter cannot infer an unverified endpoint shape. Jev category-boundary examples are rubric artifacts, and retrieval question/reference sets are qualification artifacts, under existing work, not prediction-head labels. The card's explicit overview-reference fallback preserves exact spans and full-content references within its token cap.
...
| Frozen modernbert-embed-base embedding d=768; title/abstract plus pooled original full-paper input [1536]; output/labels/masks [N,3] | Decisions 0006/0008/0010; MD-06; FT-08/FT-09 |
...
| Eight Jev original-paper assessments, smoke-tested and shown unqualified, no human reference labels or semantic head labels | Decisions 0004/0011; RD-15 to RD-24; launch profile |
...
4. Fit and qualify heads (#67), and implement and smoke-test Jev (#59 to #62). These share artifacts but keep their supervision and measurements separate.
