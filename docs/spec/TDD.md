...
| `JevScoreResult` | `type: "score"`, `field_id: JevFieldId`, `score: int`, `legend: string[n]`, `distribution: Probability[n]`, `provider_confidence: Probability\|null`. `n` is the field's scale; the score is a point on it; the legend is the question's criteria as the provider returned them; the distribution is in point order and sums to one within 0.05. |
