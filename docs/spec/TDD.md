...
| `ActionsHandlers.get_api_v1_health` | GET `/api/v1/health` | session | JSON `{state, checked_at, checks[{name, state, detail}]}`; 503 when no monitor is wired |
...
| `ActionsHandlers.get_api_v1_digests_hash` | GET `/api/v1/digests/{hash}` | Sha256 path; session | JSON digest with unrated entries reduced to `entry_id`, `paper_hash`, `display_position` and `rated: false`; 404 for an unknown digest, 503 without the inspector client or the owner's rater identity |
