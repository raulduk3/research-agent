...
The initial input is original-version title, one newline, and original abstract, UTF-8 NFC, with line breaks normalized to LF. Exclude author lists, affiliations, later metadata, references, downstream evidence and Jev fields. An empty abstract or input exceeding the selected model's token limit is unavailable; do not silently truncate. The manifest's document encoder produces one dense vector, normalized to unit L2 length and stored as float32. A zero or nonfinite vector fails. Fitting and inference use exactly these bytes and normalization. Full-text agents remain able to read the original paper; this compact head input is not claimed to represent every contribution completely.
