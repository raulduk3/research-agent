...
| forecast batch | The daily collection of forecasting questions (EN-09), routed by island and sampled for the day's coverage. |
...
| Jev assessment | A fixed, model-derived classification of what a paper reports, under the RD-16 rubric. Admitted to the launch by decision 0024 (#238), activated on the RD-22 to RD-24 evidence (SR-17). |
...
<!-- id: SDD-SR-13 | tdd: TDD-2.1.15 | status: implemented -->
...
<!-- id: SDD-SR-16 | tdd: TDD-2.1.18 | status: implemented -->
...
- Behavior: Jev paper-card assessments are admitted to the launch by decision 0024 (#238); the Jev layer is activated only on the RD-22 smoke test, RD-23 preregistration and RD-24 readiness record. Every predictive layer requires a registered baseline on the same primary measure and a comparison with the layer enabled. Pre-runtime measurements use signed, timestamped registration and result artifacts, imported with their original and import times before activation.
- Observable: The activation record resolves to a baseline registration preceding its measurement and to imported comparison evidence committed before the first activated run; the launch record shows the Jev layer activated only with its RD-24 readiness record.
...
<!-- id: SDD-SR-19 | tdd: TDD-2.1.22 | status: implemented -->
...
<!-- id: SDD-SR-20 | tdd: TDD-2.1.23 | status: implemented -->
...
<!-- id: SDD-PL-07 | tdd: TDD-2.1.35 | status: implemented -->
...
<!-- id: SDD-PL-09 | tdd: TDD-2.1.39 | status: implemented -->
...
<!-- id: SDD-PL-10 | tdd: TDD-2.1.40 | status: implemented -->
...
<!-- id: SDD-PL-15 | tdd: TDD-2.1.43 | status: implemented -->
...
<!-- id: SDD-PL-16 | tdd: TDD-2.1.44 | status: implemented -->
...
<!-- id: SDD-PL-18 | tdd: TDD-2.1.45 | status: implemented -->
...
<!-- id: SDD-PL-19 | tdd: TDD-2.1.46 | status: implemented -->
...
<!-- id: SDD-EN-01 | tdd: TDD-3.1.1 | status: pending:#65 -->
...
<!-- id: SDD-EN-35 | tdd: TDD-3.1.3 | status: pending:#73 -->
...
<!-- id: SDD-EN-37 | tdd: TDD-3.1.5 | status: pending:#65 -->
...
<!-- id: SDD-EN-38 | tdd: TDD-3.1.6 | status: pending:#65 -->
...
<!-- id: SDD-EN-07 | tdd: TDD-3.1.11 | status: pending:#65 -->
...
- Verified by: A test that runs the daily cycle over several days and checks for exactly one sealed batch per day, with questions about that day's papers only, every paper routed to one island, and exact once-per-configuration coverage of each island's sampled papers by that island's genomes. This catches a skipped day, a second batch in one day, a batch that reaches back to older papers and a paper routed to two islands.
...
<!-- id: SDD-EN-16 | tdd: TDD-3.1.17 | status: implemented -->
...
- Limits: Neither a ForeSci judgment nor a popularity count enters fitness.
...
- Limits: The trend-to-paper type of EN-24 stays unadmitted under this rule until a resolver for it is defined and accepted.
...
- Observable: The digest records each genome's ranked recommendations, rotation, selected ids and shortfalls.
- On failure: A day with no recommended paper from a genome supplies no nominations for it; fewer unique recommended papers leave fewer entries.
- Verified by: A test changes all prediction-head probabilities while keeping submissions fixed and verifies identical allocation, including repeated recommendations and shortfalls.
- Limits: Existing three random-control and two service slots remain EN-33 and EN-42, per island digest; a genome nominates only papers of its island, one per run.
...
- Behavior: The summarizer, a pinned non-evolving configuration of the agent model with the six tools disabled, receives the entry's paper card text and, for every genome of the island, the sealed probabilities and rationales for that paper and the notes of the runs that retrieved it, and writes one bounded reading: what the agents claimed, where they agreed and disagreed, and the evidence they cited. It receives no paper text, nomination, origin, genome or run identity.
...
- Limits: Every configuration in an island receives every paper of that island's daily coverage sample, drawn by hash with a recorded seed under Appendix A: Launch profile and reported as coverage; concurrency two queues excess work with deadline and missing-run accounting.
**AG-06.** Performance-based mutation must produce no child while the seeded population is fixed.
<!-- id: SDD-AG-06 | tdd: TDD-3.1.42 | status: implemented -->
...
<!-- id: SDD-AG-08 | tdd: TDD-3.1.48 | status: implemented -->
...
**AG-09.** An agent's tools must be exactly query_cards, neighbors, graph, deep_read, ask and submit.
<!-- id: SDD-AG-09 | tdd: TDD-3.1.49 | status: implemented -->

- Trigger: A run is offered its tools, and the agent model returns a tool call.
- Behavior: The loop offers the agent model these six tools and no other, less any the genome has narrowed away (AG-14). The first four read from the snapshot (AG-10), and what they return of the small models is paper card text (RD-04, RD-05). Ask has the shared tool service (PL-21), never the run, put one question to Jev about a snapshot passage or card section or the run's own words (decision 0031). Submit hands in the run's forecasts.
- Observable: The tool list in every request to the agent model names only tools among the six, and a call to any other name gets a refusal. Each answered ask is traced with the run and its request and response are stored with their hashes.
- On failure: A call to a tool outside the run's allowed set is refused with an error response. Nothing is executed and the refusal is recorded with the run. An ask about anything the run's snapshot does not hold is refused `not_in_snapshot` and Jev is not called.
- Verified by: A test in which a stand-in for the agent model calls a seventh tool name and checks that the call is refused and nothing runs, and a check of the tool list offered to the model against the six names. It catches a tool added outside the specification. A further test asks about a passage outside the run's snapshot and checks the refusal and that no request reached Jev.
- Limits: Ask's kinds, bounds and budgets are those of decision 0031 and Appendix A: Launch profile.
...
- Behavior: The call's arguments are checked against the tool's schema before the tool runs. A call that does not match exactly is refused with an error response, and its arguments are not coerced or partly used. Ask's schema has no free-text instruction field: it names what Jev reads by reference, exactly one of a passage, a card section or the run's own bounded words, and its options or scale only for the kind that uses them (decision 0031).
...
- Limits: Apply the exact context, generation, calls, deep reads, images, asks, timeout, retry, wall-time and spend ceilings in Appendix A: Launch profile.
...
- Behavior: The tools allowed in the run specification are the genome's tools when every one of them is among the six of AG-09. A genome that lists any other tool gets no run specification. A genome with ask and the same genome without it are two genomes, which is how the Jev benefit comparison treats ask (decision 0031).
- Observable: The tools allowed in every run specification are among the six, and a genome that lists any other tool has a recorded refusal and no run.
- On failure: The run specification is not built, the genome has no run on that batch, and the refusal is recorded.
- Verified by: A test that builds a run specification for a genome listing four of the six tools and checks that the run is offered only those four, and a test with a genome listing a seventh tool that checks the contract is refused. It catches a genome that gains a tool by naming it.
...
- Limits: No questions are issued for an unqualified target definition; a run without qualified targets still submits its nomination for engineering operation.
...
- Behavior: The loop attaches to every tool response the remaining amount against each budget in the run's contract (AG-12, AG-17), and against the loop's own ceilings, asks included (decision 0031), computed after the call that produced the response.
...
<!-- id: SDD-AG-28 | tdd: TDD-3.1.59 | status: implemented -->
...
- Limits: A slot is daily batch id, paper id, configuration id and attempt zero. The seed is derived from its canonical hash under Appendix A: Launch profile.
**AG-29.** The loop must record every request to the agent model and every response, by hash and in order, with the run.
<!-- id: SDD-AG-29 | tdd: TDD-3.1.62 | status: implemented -->
...
<!-- id: SDD-AG-30 | tdd: TDD-3.1.63 | status: implemented -->
...
<!-- id: SDD-AG-18 | tdd: TDD-3.1.64 | status: implemented -->
...
<!-- id: SDD-AG-19 | tdd: TDD-3.1.65 | status: implemented -->
...
<!-- id: SDD-AG-20 | tdd: TDD-3.1.66 | status: implemented -->
...
<!-- id: SDD-AG-21 | tdd: TDD-3.1.67 | status: implemented -->
...
<!-- id: SDD-AG-35 | tdd: TDD-3.1.68 | status: implemented -->
...
**AG-36.** A genome must belong to exactly one island and run only on that island's papers.
...
- Behavior: Admission requires the island part to name cs, quant-ph or q-bio. Slot creation offers a genome only the papers routed to its island's daily coverage sample (EN-09); papers of another island reach it through its tools alone and never as a slot.
- Observable: Every admitted genome names one island, and every run specification's paper belongs to that island.
- On failure: A genome with no island or an unknown one is refused and gets no run specification; a slot pairing a genome with another island's paper is not created, and the refusal is recorded.
- Verified by: A test that offers a genome without an island and one naming an unknown island and checks both refusals, and a test that builds a day's slots and checks that no genome holds a paper of another island.
...
<!-- id: SDD-AG-24 | tdd: TDD-3.1.71 | status: implemented -->
...
These assessments are admitted to the launch by decision 0024 (#238), which replaces the hold-out of decision 0015 (#123); provider access was verified under #59. The requirements below keep their ids and text. The Jev layer is activated only on the RD-22 smoke test, RD-23 preregistration and RD-24 readiness record, which the execution gates #60 to #62 produce, and both arms of RD-23 run.
...
- Limits: The exact profiles and thresholds are fixed in Appendix A: Launch profile. The assessments are admitted to the launch by decision 0024 (#238), so launch readiness requires this record; provider access was verified under #59, and source access, provider permissions, the smoke test and authorized funding are execution gates under #60 to #62.
...
- Verified by: A test rejects a wrong dimension, revision or compute platform and verifies the exact document/query prefixes and the [1536] embedding feature vector.
...
- Limits: X has shape [N,2d+m], the embedding block followed by the declared metadata block of Appendix B: Learning protocol; probabilities, labels and masks have shape [N,3]. Independent models do not imply independent outcomes.
...
<!-- id: SDD-FT-13 | tdd: TDD-4.1.76 | status: implemented -->
...
<!-- id: SDD-FT-14 | tdd: TDD-4.1.77 | status: implemented -->
...
<!-- id: SDD-FT-15 | tdd: TDD-4.1.78 | status: implemented -->
...
<!-- id: SDD-FT-27 | tdd: TDD-4.1.81 | status: implemented -->
...
The eight Jev content assessments are admitted to the launch by decision 0024 (#238), which replaces the hold-out of decision 0015 (#123). RD-15 to RD-24 keep their ids and text. Under SR-17 the Jev layer is activated only on the RD-22 smoke test with owner review, the RD-23 preregistration and the RD-24 readiness record, which #60 to #62 produce; provider access was verified under #59. Both arms of RD-23 run. Each paper card carries every assessment field or its RD-18 unavailable state, and the funded Jev sublimit stays unused until funded execution is authorized.
...
Docker Compose declares local components, private networks, volumes, health checks, seccomp defaults, read-only root filesystems and per-service runtime secrets. Agent workers have no Docker socket or privileged mode. Host-enforced egress permits ingest to the recorded arXiv/OpenAlex/source allowlist and each worker, including a summarizer call, to the one named provider's API host in Pinned model choices below, through an authenticated proxy; no second agent-model host, relay or fallback host is reachable. The Jev entry in the ingest allowlist is the one gateway path of RD-20 (decision 0024, #238); no second Jev host is reachable. Private application calls follow explicit service allowlists. Storage alone reaches the backup/anchor endpoint. Provisioning tools run outside agent containers under the operator's authority. Neither the rating app nor workers can reach arbitrary internet hosts. Local HTTPS binds only to a LAN/private-VPN address. There is no public listener for the rating app, storage, models, tools or any internal route (decision 0030, #336). The owner surfaces alone, the owner actions app, the inspector and the owner reads of `/api/v1/`, each requiring an owner session on every route, may be published through one ingress: a reverse proxy pinned by digest, the only service with a port, bound to the guest's loopback, reached by one authenticated tunnel whose public hostname the profile records and which terminates TLS. The ingress refuses to start the tunnel if any route it would publish is not owner-authenticated, and the owner API admits cross-origin credentialed requests from the one front-end origin the profile names and from no other. Two operator-provisioned rater identities, each bound to one rated island, use hashed credentials and secure HttpOnly SameSite=Strict sessions with CSRF checks, 24-hour expiration and no open registration.
...
The scheduler runs at most one memory-heavy batch job, pauses it while foreground services exceed three quarters of the guest's memory in resident memory, and resumes from checkpoints below five eighths of it (48 and 40 GiB of a 64 GiB guest). The thresholds are fractions of the Linux guest's memory, not the host machine's, and the guest's vCPUs and memory are the profile's, not a committed default (decision 0030, #336). CPU limits are ceilings, not promised simultaneous reservations. Readiness measures latency, peak memory and full daily completion on the actual host; the floor alone is not a performance guarantee. Health polling is every 30 seconds, with three failed polls marking a service failed; initial model load gets 15 minutes. Service failures retry at most three starts, 10/30/90 seconds apart, then require operator repair. A failed optional provider does not prevent capture or core paper-card reading.
...
Default `paid_execution_enabled=false`, active monetary authorization is zero. Maximum ceilings after explicit funding authorization: combined paid spend USD 8 per UTC day and USD 200 per UTC month, Jev sublimit USD 2/day retained but unused until funded execution is authorized (decision 0024, #238), scholarly API sublimit USD 2/day. Inside the Jev sublimit, run-time asks (decision 0031) draw from a daily pool of USD 0.50; each ask reserves its worst case against the pool and the whole sublimit in the same record that reserves card assessments, so asks never spend what the sublimit leaves the cards, and past the pool an ask is refused `daily_ask_budget_exhausted`. Reserve worst-case request charges before execution against both counters; provider pricing/quote must be dated and available. If the quote, cost bound or authorization is missing, do not call. Ambiguous billed attempts consume their reserved amount until reconciled. Nothing in this design provisions, restarts or purchases a resource.
...
Before activation, execute 100 real six-tool/schema conversations (decision 0031) with at least 99 valid complete submissions and no successful forbidden tool; 50 preserved scientific figure/table questions with at least 80% independently agreed correct answers; and 100 evidence-location questions balanced over early/middle/late positions in 16k/32k/64k contexts with at least 90% correct cited source ids at each tested length. These are operating acceptance floors. Use separate held-out cases after fixes. Measure load time, peak memory, concurrent requests, full daily duration and loss/retry behavior. No provider benchmark substitutes for these tests.

The selected embedding model is a baseline candidate, not a claim that modernbert-embed-base beats SciEmbed. Require artifact/license/input tests and the retrieval evaluation below before study use. The release-1 prediction heads are fitted on the winner of the required comparison below; the overview slot and the passage slot of the head input may come from different representation namespaces when the comparison favors it, and each namespace keeps the full-rebuild and never-mix rules of #36. Replacement under #36 uses a new representation namespace, rebuilds every affected historical vector and prediction head, requalifies retrieval and prediction heads, and atomically switches only future snapshots. Never mix vector spaces. Contemporary embeddings can fit reconstructed historical labels with disclosed pretraining limitations; no unsupported exclusion based on a guessed training cutoff, and no retrospective foresight claim. Known cutoff metadata is recorded; unknown stays unknown.

<a id="launch-profile-agent-batches-schemas-and-bounded-reading"></a>
### Agent batches, schemas and bounded reading

Ingest refreshes arXiv at 00:00 UTC daily, creates one daily parent batch after completion and records per-paper publication/arrival lateness. Route each eligible family to the island of its primary category, cs for cs.AI and cs.LG, quant-ph and q-bio. Within each island sort families by first_public_at then canonical id, and draw that island's daily coverage sample by ascending SHA-256 of the canonical JSON object {utc_day, island, family_id, seed} with a recorded seed, taking the largest prefix of that order the island's share of the remaining authorized spend covers at the measured per-run cost for each of its configurations, shares in proportion to each island's papers that day; every genome of the island reads the same sample, and the day's coverage is reported. Each island's questions on the day's papers are sealed as sheets of at most 20 questions holding whole papers, and the day's snapshot is sealed after them and pinned to each (decision 0027). Every configuration of an island receives one slot per sampled paper, with the same questions and snapshot; no paper's slot mixes islands. A slot is (daily_batch_id, paper_id, configuration_id, attempt=0), where daily_batch_id is the hash of the sealed sheet holding the paper's questions. The population runs at most two configurations concurrently; queue by earliest paper seal deadline then slot id. Every scheduled slot has a completion, void or missed-deadline record. Papers outside the sample keep their cards and head probabilities and receive no agent forecast that day. Sampling changes forecast coverage, not paper inclusion in the corpus or the 24-hour first-public forecast deadline. Prediction-head/assessment acquisition must complete before the snapshot; later enrichment produces future paper-card versions only.

The first request contains the run's paper id, its issued questions, budgets and snapshot description, not a preloaded paper card. The agent chooses its reads through the six tools of AG-09. The seeded population is three islands of four configurations each, twelve in all: in every island the four launch emphases, which are evidence-first reading, methods/assumptions scrutiny, comparison to earlier related work and limitations/alternative-explanation scrutiny, with the evidence-first configuration marked as the island's founder (AG-38); owner-written variants are admitted under AG-03 before the first cycle into a named island. All use identical tools, budgets, model, rubric and target definitions. Each prompt begins with the same instruction to treat source content as evidence rather than instructions, assess what the paper supports, separate reading preference from citation forecasts, and acknowledge missing evidence. Configuration differences are only the named reading emphasis. Prompts are versioned immutable artifacts frozen before evaluation; wording changes invalidate the configuration identity. No claim that any emphasis is optimal is assumed.

Per run: 6 model calls, 12 total tool calls including rejected calls, 3 deep reads, 6 images, 4 asks (decision 0031), 32768 model tokens maximum context, 4096 total generated tokens, a 64000 max_tokens_per_run ceiling on cumulative context and generated tokens across the run, and 5 minutes wall time. Per request reserve up to 2048 output tokens inside the context limit; maximum 2048 generated tokens for one response. Resent input counts toward measured usage and the spend reservation; model-token counts include images using the pinned processor. No implicit compression, hidden summarization or dropped conversation turns. A response exceeding remaining allowance is not executed; expiration before an accepted submit is void. Provider request timeout 120 seconds. Retry only explicitly rejected, non-executed 429/503 requests once after 5 seconds within budgets; unknown completion/eviction stops the run as void, without selecting a favorable retry. Idempotent submit retries are allowed from the recorded identical request. A stopped run cannot resume after its seal deadline. The launch profile's `run` section carries these per-run values with the six tools, the 120-second request timeout and a per-run spend reservation of USD 0.01, the worst case at the recorded prices under these limits; a settled run not yet priced counts its full reservation against the daily and monthly caps (#285). These are configured values, measured before activation like every other value in this appendix.

Each turn's protected note is a concise action/evidence summary, at most 1000 UTF-8 characters, not a demand for private chain-of-thought. Intent enum: scan, compare, inspect, forecast, nominate, submit, stop. Forecast rationale maximum 2000 characters, with at most five source ids. The launch schema extension is empty; reject any extra field. Reserved future extension types are string<=1000 chars, finite number, boolean, enum<=16 choices and array<=8 primitive values, maximum eight fields and depth two, labels<=80 chars and descriptions<=240 chars; none can activate without the deferred schema-evolution decision. Scan/read/probability policies are UTF-8 text <=4000 characters each, overall system prompt <=16000 characters, tools a subset of the fixed six, one sample per question. The external loop validates one sample rather than fabricating independent samples from prose.
...
Engineering shards with no issued forecast questions expire 24 hours after batch sealing. Population and independent with/without-Jev comparison slots share global concurrency and spending limits; the comparison adds two slots per eligible shard beside the population slots and cannot contribute digest nominations. Comparison slots are created only once the RD-23 comparison is registered (decision 0024, #238). Deep-read continuation can only resume a returned immutable span within the same snapshot and paper.
...
The overview input is original-version title, one newline, and original abstract, UTF-8 NFC, with line breaks normalized to LF. An empty abstract or input exceeding the selected model's token limit is unavailable; do not silently truncate. Encode the overview and original full-text passages using Appendix C: Retrieval protocol. Concatenate the normalized overview and overlap-weighted normalized passage pool, divided by sqrt(2), to obtain one float32 embedding vector of shape [2d]. Reject missing complete original-text coverage, zero/nonfinite vectors or incompatible representations.

The fitted prediction-head input appends a fixed, declared metadata block of width m to the embedding vector, giving one float32 vector of shape [2d+m]: author count (log1p), the count of listed categories, a one-hot of the primary category set, abstract token count (log1p), title token count, a one-hot of the first-availability weekday, whether the abstract or comments name a code repository, and the version count at seal, in this closed order; adding a feature is an amendment. Every metadata feature is available at seal time from the paper card and nothing else; a card missing one refuses the row rather than substituting a zero or falling back to the embedding alone. The metadata block is standardized per target and category from the fitting partition's mean and standard deviation, stored on the head bundle and applied identically at fitting, calibration, promotion and inference; a bundle without a stored standardization is refused. The embedding block's unit-L2 invariant applies to its own [2d] prefix only. Exclude citation counts, downstream evidence and Jev fields from both blocks. Author or result cues embedded in the original text are not claimed to be removed. Fitting and inference share exactly this construction. Record retrieval availability separately: partial text can be retrieved even when prediction-head features are unavailable.
...
Training arrays: X float32 [N,2d+m], Y boolean [N,3], M boolean [N,3], ordered paper ids and the immutable target/representation manifests. Y entries with M=false have no semantic value and never enter a loss. Each prediction-head fits only rows with its own M=true. Inference accepts original paper/version id and pinned bundle id, constructs x [2d+m], and returns three named records. Internal probabilities have shape [N,3] with an availability mask; external unavailable values are null, not zero or NaN.
...
Reuse original extraction and embeddings across targets and refreshes. The Jev smoke test (RD-24) is a separate gate for the assessments admitted by decision 0024 (#238); automatic head labels do not replace it.
