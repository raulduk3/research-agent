...
| assessment confidence | The certainty a model reports for a Jev assessment, summarized from its returned class distribution (RD-18). It is not a forecast probability and not measured accuracy (RD-22). |
...
- Observable: Each paper card labels the assessment source and rubric version separately from forecasts and measured counts.
...
- Verified by: A test rejects wrong dimension/revision and verifies the exact document/query formatting and [2048] prediction-head input.
...
The selected embedding model is a baseline candidate, not a claim that Qwen beats SciEmbed. Require artifact/license/input tests and the retrieval evaluation below before study use. Replacement under #36 uses a new representation namespace, rebuilds every affected historical vector and prediction head, requalifies retrieval and prediction heads, and atomically switches only future snapshots. Never mix vector spaces. Contemporary embeddings can fit reconstructed historical labels with disclosed pretraining limitations; no unsupported exclusion based on a guessed training cutoff, and no retrospective foresight claim. Known cutoff metadata is recorded; unknown stays unknown.
...
Reuse original extraction and embeddings across targets and refreshes. Jev content-assessment qualification is a separate launch gate (RD-24); automatic head labels do not replace it.
