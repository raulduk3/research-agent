...
A disposable Linux VM, defined in `lima-collection-boundary.yaml`, provides limited container execution evidence; see `linux-boundary-evidence.md`. The pinned PostgreSQL image runs and rejects the tested forbidden SQL operations. The committed Dockerfile now builds and serves under Rosetta translation in that guest, and a worker container there reached neither PostgreSQL, the authoritative artifact path, the Docker socket nor an unlisted live destination. Host firewall enforcement is still unproven, so collection admission remains refused. `bin/check-collection-linux` is a separate explicit gate; an unavailable result is not a passing test. No existing service was restarted. No paid jobs, purchases, deployment, merge, or unrelated worktree changes were performed.
