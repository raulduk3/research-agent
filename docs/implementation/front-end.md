...
chips, page links, reader turns, read tiles, paper days and question cards may number zero or
...
| Islands | `islands.html` | the papers each island reads, its rater, its budget share and its selection proxy, which are the launch profile's and not a stored record, and the identifier fold's launch profile hash. Each island of the launch profile has a row, then any other island the read names; `/api/v1/islands` gives its stored agents with their founders and lineages, its runs (all-time, where the mock counts papers this month) and its latest run instead of today's reads. An island the read does not name has no agent stored. Without the read the table has no rows |
| Island | `island.html` | the island's paper stream, rater, budget share and room, its selection state, today's papers and digest, the accept rate, the recent weeks, the agents' forecasts, rater credit and agreement with the heads, and the replay (#208), whose controls are kept but disabled. `/api/v1/islands/{island}` gives its stored agents, founders first, each with its runs, void runs, cost per priced run and latest run, and the cards sum them: all-time runs with the void ones, and settled cost over the priced runs, where the mock gives today's papers and spend. The identifier fold lists each agent's configuration hash. The explore links lead to the swarm, reports, runs and owner home pages. Without the read the table has no rows and the cards read "not served yet" |
...
| Reports | `reports.html` | each report's state, headline and selection, because a report is built on request and not stored, and the identifier fold's preregistration and launch profile hashes. `/api/v1/reports` gives one row per island and ISO week holding a stored digest or rating, newest first, each opening its report; the State column gives that week's stored digests, entries, ratings and credits. The week is written Monday to Sunday, where the mock shortens the first week to the study days. Without the read the table has no rows |
| Questions | `questions.html` | the rater's own calls, the paper's title, guide and arXiv link, and the day each question was asked. The mock is a rater's page; a human forecast is sealed with its rater as submitter and an owner session carries no rater id, the paper is inside its card artifact, and no route takes a forecast. `/api/v1/questions` gives each question a sealed sheet holds, named by its resolver and version, with its horizon, sheets, runs and submissions: open questions (nothing resolved) as cards, then the resolved ones in the fold with their true, false and unresolvable counts and latest resolution. Each card opens its question page. The identifier fold lists each question id and target definition hash. The cards are list items (`LIST_ITEMS`). Without the read there are no cards |
| Question | `question-Q1.html` | the rater's call and its guide, the paper (title, parts map, PDF and its own page), the evidence the reads cited, the summarizer's reading, the baselines, the authors, the content assessment and the question's hour-by-hour replay (#208). The five panels show one at a time, chosen by the tabs and step links; the mock locks three of them until the rater's call, and an owner session, which has no call, reads them all, so the tabs keep the mock's `locked` class and nothing is withheld. `/api/v1/questions/{question_id}` gives the question named by its resolver and version, its horizon and sheets, each run that submitted a chance on it (the conversation's turns, linked to the agent and to the run's tool calls, and the read tiles, linked to the run), and its current resolutions, counted in the life panel and listed by forecast id in the identifier fold. The question read carries no paper id, so the paper and its PDF are not served here. Without the read the turns and tiles are empty |
| Impact | `impact.html` | the rater's own calls (an owner session carries no rater id, so the page counts every rater's), the rate at which agents' picks were accepted against hidden random controls and its interval, the forecasts a rater sealed, the papers left uncalled (no record holds an uncalled paper) and changing a call (a call cannot yet be changed). `/api/v1/impact` gives each island and rating week's ratings by value, credit rows, genomes credited and credit gaps; the facts sum the latest week's rows over its islands, with its accept rate (likes over ratings) as a chance bar. The chain and the limits of a call are the mock's explanation, without its dates. Without the read the facts read "not served yet"; with no rating stored they say so |
...
| Paper | `paper-P1.html` | the title, abstract and arXiv link, the parts map, the rater's call, the summarizer's reading, the baselines, the authors, the text of the content assessment, the paper's days and their replay (#208). The five panels show one at a time, chosen by the tabs and step links. The conversation is one turn per run with its chance on each question, by agent id, since the owner sees who read it; the read tiles are each forecast's reason, linked to the run's tool calls. The paper's requests, pinned cards and embedding are its record page (`/papers/:paperId/record`), linked from the "More" fold. The mock's `reading-*.html` pages are each run's own page (`run.html`). The PDF link and frame open the newest of the retained PDFs `/api/v1/owner/papers/{paper_id}/documents` lists for the family's pinned cards, at `/api/v1/owner/documents/{artifact_hash}` (`application/pdf`); with none retained the link says so, and without the read it reads "not served yet". The owner paper read's cards carry the snapshot, version and card hashes only; the title, abstract and arXiv id are inside the card artifact and are not served as fields. The content assessment line names, by hash, the distinct sections the pinned cards' snapshots pin (`assessment_section_hash`), or says none is pinned; the section's text is not served. The rater's call is stored (`ratings`, keyed by the digest entry's paper hash, with no stored link to a paper family) and is not served: the rating guard withholds other raters' figures from an owner session, so serving it needs the owner's decision. The summarizer's reading, rater flags, the baselines and the authors have no stored table and are not served |
| Model | `models.html` | models other than agent models, the prediction heads and their calibration, the training corpus, the agent model, the summarizer, spending and the content assessments. The heads' own page (`head.html`) has no route either. The model list, on the menu entry and above each manifest, reads `/api/v1/models`: each agent model manifest a stored run pins, as "agent model" with its run count and first and latest run instants, each opening its manifest; the menu entry still opens any other manifest by its hash. The heads, their calibration, the training corpus, the summarizer, spending and the content assessments are not listed; the manifest takes the embedding section's place, its kind as the heading and its fields as the table. |

Mock pages with no `/api/v1` route at all (`docs/contracts/api-v1/endpoints.json`) are not ported: `swarm.html`. Paths without a ported page render `src/pages/NotServed.tsx`.
