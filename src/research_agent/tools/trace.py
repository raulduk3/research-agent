...
the artifact ids the answer showed the run and its budget deltas. Nothing
the agent writes reaches these fields, so a check of the run's conduct
reads what the service did, never what the agent says it did.
...
__all__ = ["TraceEntry", "RunTrace", "TraceAppends", "TraceWriter", "request_hash"]
...
def request_hash(
    *, run_id: str, snapshot_id: str, tool: str, raw_arguments: object
    ...
    """The canonical hash of one call exactly as the run made it.

    Arguments that have no canonical JSON form -- which admission refuses --
    are hashed by their representation, so even such a call has an entry.
    """
    ...
    try:
        return sha256_hex(canonical_json(call))
    except CanonicalJsonError:
        return sha256_hex(repr(call).encode("utf-8", "backslashreplace"))
...
class TraceWriter:
    ...
    def request(
        ...
        *,
        ...
        """Record a call before it runs; return its call sequence.

        *refusal* is the admission's refusal code, ``None`` for an admitted
        call. A tool name no trace row can hold is recorded as ``invalid``:
        it was refused as agent text before any tool could run.
        """
    ...
    def terminal(
        ...
        *,
        ...
        self._storage.append_trace_terminal(
            ...
            response_hash=sha256_hex(canonical_json(envelope)),
